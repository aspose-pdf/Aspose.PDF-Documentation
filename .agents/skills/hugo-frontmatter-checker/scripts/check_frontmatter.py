...
def scan(
    ...
    for md in files:
        ...
        rel = md.relative_to(root) if root.is_dir() else Path(md.name)
        parts = rel.parts
...
def main() -> None:
    ...
    if root.is_file() and root.suffix.lower() != ".md":
        print(f"Error: '{root}' is not a Markdown file", file=sys.stderr)
        sys.exit(2)
