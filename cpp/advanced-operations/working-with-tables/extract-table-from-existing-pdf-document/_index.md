...
void ExtractTableBorderAsImage()
{
 ...
 for (auto op : document->get_Pages()->idx_get(1)->get_Contents())
 {
  ...
  else if (opLineTo != nullptr)
  {
   ...
   graphicsPath->AddLine(linePoint->get_X(), linePoint->get_Y(), lastPoint->get_X(), lastPoint->get_Y());

   lastPoint = linePoint;
  }
  else if (opRe != nullptr)
  {
   auto re = MakeObject<System::Drawing::RectangleF>((float)opRe->get_X(), (float)opRe->get_Y(), (float)opRe->get_Width(), (float)opRe->get_Height());
