...
四角形および円形の注釈は、それぞれページ上に長方形または楕円を表示します。開くと、関連するメモのテキストを含むポップアップウィンドウが表示されます。四角形の注釈は、形状を除いて円形の注釈（Aspose.Pdf.Annotations.CircleAnnotationクラスのインスタンス）と似ています。
...
1. 新しい[PopupAnnotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.popup_annotation)を作成します。
1. 次に、[Square Annotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.square_annotation)を作成する必要があります。
1. 同じ四角形のパラメーター（新しい長方形、タイトル、色、内部色、不透明度）を設定します。
1. その後、ページに四角形と円のアノテーションを追加する必要があります。

次のコードスニペットは、PDFページに円形アノテーションを追加する方法を示しています。
...
```
以下は、PDFドキュメントに四角形と円形の注釈を追加した結果の例です。

![Circle and Square Annotation demo](circle_demo.png)

### 円形注釈を取得

PDFドキュメントから円形注釈を取得するために、次のコードスニペットを使用してください。
...
PDFファイルから円形注釈を削除する方法を示すコードスニペットは次のとおりです。
...
1. PDFファイルをロードする - new [Document](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.document).
1. 新しい [Polygon Annotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.polygon_annotation) を作成し、ポリゴンのパラメータを設定します (新しいRectangle、新しいPoints、タイトル、色、InteriorColor、Opacity)。
1. 新しい [PopupAnnotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.popup_annotation) を作成します。
1. 次に、[PolyLine Annotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.polyline_annotation)を作成し、すべての操作を繰り返します。
1. その後、ページに注釈を追加できます。
...
ライン注釈の目的は、ページ上に1本の直線を表示することです。開いたときに、関連するメモのテキストを含むポップアップウィンドウを表示します。
この機能は、ライン注釈に特有の追加のエントリを持っています。これらのエントリは、例えばLL、BS、ICなどの文字の形で暗号化されています。

また、ライン注釈にはキャプションを含めることができ、これはCapを`true`に設定することで指定されます。
次の機能では、リーダーオフセットを持つライン注釈にキャプションを適用する効果を許可します。
また、この種の注釈では、ラインの終端スタイルを定義することができます。

ライン注釈を作成する手順：

1. PDFファイルをロードする - 新しい[Document](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.document)。 新しい[Line Annotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.line_annotation)を作成し、ラインのパラメータ（新しいRectangle、新しいPoint、タイトル、色、幅、StartingStyleおよびEndingStyle）を設定します。
1. 新しい[PopupAnnotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.popup_annotation)を作成します。
1. その後、注釈をページに追加できます。
...
PDFドキュメントでライン注釈を取得するには、以下のコードスニペットを試してください。
...
次のコードスニペットは、PDFファイルからライン注釈を削除する方法を示しています。
...
void DeleteLineAnnotation() {
    ...
    // AnnotationSelectorを使用して注釈をフィルタリングします
    auto page = document->get_Pages()->idx_get(1);
    auto annotationSelector = MakeObject<Aspose::Pdf::Annotations::AnnotationSelector>(
        MakeObject<Aspose::Pdf::Annotations::LineAnnotation>(page, Rectangle::get_Trivial(), Point::get_Trivial(), Point::get_Trivial()));
    page->Accept(annotationSelector);
...
インク注釈は、1つまたは複数の不連続なパスで構成されるフリーハンドの「落書き」を表します。 開いたとき、関連するノートのテキストを含むポップアップウィンドウを表示します。

[InkAnnotation](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.annotations.ink_annotation) は、1つ以上の非連続のポイントで構成される手書きの落書きを表します。PDFドキュメントにInkAnnotationを追加するには、次のコードスニペットを使用してください。
...
void ShapesAnnotations::AddInkAnnotation() {
    ...
    //マウスやその他のポインティングデバイスから受信したpptsのデータ
...
PDFドキュメントでInkAnnotationを取得するために、以下のコードスニペットを試してください。
...
PDFファイルからInkAnnotationを削除する方法を以下のコードスニペットで示します。
