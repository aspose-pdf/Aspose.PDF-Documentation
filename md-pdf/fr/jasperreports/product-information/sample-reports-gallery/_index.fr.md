```
title: Galerie de Rapports d'Échantillons

type: docs

weight: 40

url: /jasperreports/sample-reports-gallery/

lastmod: "2021-06-05"

---

{{% alert color="primary" %}}

Cette galerie présente des rapports PDF exportés par Aspose.PDF pour JasperReports.

{{% /alert %}}

Les rapports présentés ci-dessous sont basés sur des données d'échantillon installées avec JasperServer.

**Rapport de tous les comptes**

![todo:image_alt_text](sample-reports-gallery_1.png)

**Rapport des magasins Supermart**

![todo:image_alt_text](sample-reports-gallery_2.png)

**Rapport Aegean de graphiques standards**

![todo:image_alt_text](sample-reports-gallery_3.png)

**Rapport Aegean de graphiques standards**

![todo:image_alt_text](sample-reports-gallery_4.png)

**Rapport de graphiques standards attrayants**

![todo:image_alt_text](sample-reports-gallery_5.png)

**Rapport de graphiques standards attrayants**

![todo:image_alt_text](sample-reports-gallery_6.png)

**Rapport de graphiques standards attrayants**

![todo:image_alt_text](sample-reports-gallery_7.png)

**Rapport de graphiques standards**
```

```
![todo:image_alt_text](sample-reports-gallery_8.png)

**Rapport de graphiques standard**

![todo:image_alt_text](sample-reports-gallery_9.png)
```