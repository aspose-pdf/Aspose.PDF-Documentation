...
Генерация PDF документа из XML документа является нетривиальной задачей, потому что XML документ может описывать различный контент. Aspose.PDF for C++ имеет несколько способов генерации PDF на основе XML документа:

- использование XSLT трансформации
- использование XSL-FO (XSL Formatting Objects) разметки
- использование собственной XML схемы Aspose.PDF
...
Итак, нам нужно преобразовать XML и загрузить в PDF документ. Следующий пример показывает этот способ:
...
void WorkingWithXML::ExampleXSLTtoPDF()
{
 ...
 // установите размер страницы на A5
...
XSL-FO — это основанный на XML язык разметки, описывающий форматирование данных XML для вывода на экран, бумагу или другие носители. Aspose.PDF имеет специальный класс, который позволяет применять разметку XSL-FO и получать PDF документ.
...
Давайте создадим еще один файл — файл разметки XSL-FO для преобразования данных о сотрудниках в таблицу.
...
Aspose.PDF имеет специальный класс [XslFoLoadOptions](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.xsl_fo_load_options/), который позволяет применять XSL-FO трансформацию. Следующий фрагмент показывает, как использовать этот класс с вышеописанными примерами файлов.

```cpp
void WorkingWithXML::Example_XSLFO_to_PDF()
{
 String _dataDir("C:\\Samples\\");
 // Создаем объект XslFoLoadOption
...
```
Чтобы добавить параметры XSL, нам нужно создать свой собственный 'XsltArgumentList' и установить его как свойство в [XslFoLoadOptions](https://reference.aspose.com/pdf/cpp/class/aspose.pdf.xsl_fo_load_options/). Следующий фрагмент показывает, как использовать этот класс с описанными выше примерами файлов.
...
Другой способ создать PDF документ из XML — использовать Aspose.PDF XML Schema. Используя эту диаграмму, вы можете описать макет страницы так же, как если бы вы использовали табличный макет в HTML. Давайте рассмотрим работу этого метода более подробно.

### Определение страницы

Давайте определим страницу с параметрами по умолчанию. Наша страница будет иметь формат A4 и содержать только один фрагмент текста.
...
Чтобы определить новый размер страницы, мы должны добавить элемент `PageInfo`. В следующем примере мы установили размер страницы A5 и поля 25мм и 10мм.
...
Поскольку HTML содержит теги, похожие на XML, то, когда вы пишете HTML внутри любого XML тега, парсер рассматривает его как XML разметку, и они просто не могут быть распознаны как XML теги. Проблема может быть решена с помощью секции "CDATA" в XML. Секция CDATA содержит текст, который не анализируется парсером или, другими словами, не рассматривается как XML-разметка. Следующий пример шаблона XML показывает, как добавить HtmlFragment внутрь XML-разметки, используя CDATA.
...
1. Выравнивание по ширине. Текст будет выровнен по левому и правому полям.
1. 
Полное выравнивание. Похоже на выравнивание "по ширине", за исключением того, что последняя строка будет выровнена только по левому краю в режиме "по ширине", в то время как в режиме "Полное выравнивание" все строки будут выровнены по левому и правому краю.
...
```
```
Таблицы используются для оформления документов. Например, мы можем настроить верхний колонтитул страницы. В этом случае таблица использовалась для разделения заголовка на 2 колонки.
...
Метод BindXML() предлагает возможность загружать содержимое файла XML, а метод Document.save() может быть использован для сохранения результата в формате PDF. Однако в процессе конверсии мы также можем получить доступ к отдельным элементам внутри XML и использовать XML в качестве шаблона. Следующий фрагмент кода показывает шаги для доступа к TextSegments из XML-файла.
...
### Установка пути к изображению при преобразовании XML в PDF

Следующий XML шаблон содержит в себе тег `<Image>` с идентификатором "testImg". В случае, если вы хотите установить путь к изображению из вашего кода, вы можете получить доступ к элементу Image из XML шаблона в процессе преобразования и установить путь к вашему желаемому адресу для изображения.
