...
print "Rect : Height = " + pdf_page.getRect().getHeight() + ", Width = " + pdf_page.getRect().getWidth() + ", LLX = " + pdf_page.getRect().getLLX() + ", LLY = " + pdf_page.getRect().getLLY() + ", URX = " + pdf_page.getRect().getURX() + ", URY = " + pdf_page.getRect().getURY() print "페이지 번호 :- " + pdf_page.getNumber()
print "회전 :-" + pdf_page.getRotate()

```

**실행 코드 다운로드**

아래 언급된 소셜 코딩 사이트 중 하나에서 **Get Page Properties (Aspose.PDF)**를 다운로드하세요:
