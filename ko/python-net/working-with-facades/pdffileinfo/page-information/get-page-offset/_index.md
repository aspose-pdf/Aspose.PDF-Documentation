...
Abstract: 'get_page_offsets' 함수는 PDF 파일에 있는 각 페이지의 가로 (X) 및 세로 (Y) 오프셋을 추출합니다.이러한 오프셋은 PDF 원점을 기준으로 한 페이지 내용의 위치를 나타냅니다.이 함수는 포인트를 인치로 변환하여 주석, 이미지 또는 텍스트를 정확하게 배치하는 데 사용할 수 있는 정확하고 사람이 읽을 수 있는 측정값을 제공합니다.여러 페이지로 구성된 PDF를 지원하며 PDF 레이아웃, 자동화 또는 콘텐츠 정렬 작업을 수행하는 개발자를 위한 것입니다.
---
