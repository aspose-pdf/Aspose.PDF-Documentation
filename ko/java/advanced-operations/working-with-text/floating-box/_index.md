...
테두리가 있는 부동 컨테이너 안에 텍스트를 배치해야 하는 경우 이 예를 사용하십시오.
...
긴 텍스트가 하나의 부동 상자 내 여러 열에 걸쳐 흘러야 하는 경우 이 예를 사용하십시오.
...
1. 반복되는 텍스트 조각을 상자에 추가하고 문서를 저장합니다.
...
1. 
페이지를 생성하고 다중 열 `FloatingBox`을 구성합니다.

1. 
텍스트 조각을 생성하고 `setFirstParagraphInColumn(true)`으로 표시합니다.
1. 페이지에 상자를 추가하고 PDF를 저장합니다.
