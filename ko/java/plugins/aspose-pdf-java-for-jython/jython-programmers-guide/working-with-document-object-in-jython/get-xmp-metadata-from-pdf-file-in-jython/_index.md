---
title: Jython의 PDF 파일에서 XMP 메타데이터 가져오기
linktitle: Jython의 PDF 파일에서 XMP 메타데이터 가져오기
type: docs
weight: 40
url: /java/get-xmp-metadata-from-pdf-file-in-jython/
description: 자세한 콘텐츠 분석을 위해 Aspose.PDF와 함께 Jython을 사용하여 PDF 문서에서 XMP 메타데이터를 추출하는 방법을 살펴보세요.
lastmod: "2026-06-09"
---
## 
Aspose.PDF - XMP 메타데이터 가져오기



**Aspose.PDF Java for Jython**을 사용하여 파일 형식을 확인하려면. 여기에서 샘플 코드를 볼 수 있습니다.

매크로 'code' 렌더링 오류: 매개변수 lang에 잘못된 값이 지정되었습니다.


## 
실행 코드 다운로드



아래에 언급된 소셜 코딩 사이트 중 하나에서 실행 코드를 다운로드하세요.


- 
[GitHub](https://github.com/aspose-pdf/Aspose.PDF-for-Java/releases)
