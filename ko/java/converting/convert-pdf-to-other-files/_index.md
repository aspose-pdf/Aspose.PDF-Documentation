...
Aspose.PDF for Java는 PDF 문서를 텍스트, 전자책, 인쇄 및 마크업 지향 출력 형식으로 내보낼 수 있습니다.
...
1. 
PDF 콘텐츠를 리플로우 가능한 EPUB 마크업으로 내보내려면 `document.save(outputFile.toString(), saveOptions)`으로 전화하세요.
...
1. 
TeX 직렬화를 위해 [`TeXSaveOptions`](https://reference.aspose.com/pdf/java/com.aspose.pdf/texsaveoptions/)을 만듭니다.

1. 
PDF 콘텐츠가 TeX 마크업으로 내보내지도록 `document.save(outputFile.toString(), saveOptions)`을 호출하세요.
...
1. 
첫 페이지를 일반 텍스트로 작성하려면 `device.process(document.getPages().get_Item(1), outputFile.toString())`으로 전화하세요.
...
1. 
`document.save(outputFile.toString(), SaveFormat.MobiXml)`으로 전화하면 PDF가 Mobi 호환 XML로 내보내집니다.
