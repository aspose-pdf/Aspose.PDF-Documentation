...
Abstract: Java용 Aspose.PDF를 사용하여 PDF 메타데이터를 검색하는 방법을 알아보세요. Java 예제는 파일 상태 플래그 및 사용자 정의 `Reviewer` 메타데이터 항목과 함께 제목, 제목, 키워드, 작성자, 생성 날짜 및 수정 날짜와 같은 표준 필드를 읽습니다.
---
## 
PDF 메타데이터 가져오기
...
3. 
파일이 유효한지, 암호화되었는지, 비밀번호로 보호되었는지, 포트폴리오인지 등의 파일 상태 플래그를 검사합니다.
