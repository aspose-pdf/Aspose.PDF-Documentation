...
Abstract: 이 섹션에서는 현재 Java PdfContentEditor 예제 세트에서 지원되는 이미지 관련 작업 흐름을 다룹니다. 저장소에는 이미지 교체에 대한 직접적인 예가 포함되어 있으며, 지원되지 않는 이미지 삭제 항목은 명시적인 범위 참고 사항으로 유지됩니다.
---
...
2. 
페이지 번호, 이미지 인덱스, 대체 이미지 경로를 `replaceImage(...)`으로 전화하세요.
