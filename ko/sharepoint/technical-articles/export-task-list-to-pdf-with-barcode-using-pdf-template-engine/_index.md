---
title: PDF 템플릿 엔진을 사용하여 작업 목록을 바코드가 있는 PDF로 내보내기
linktitle: PDF 템플릿 엔진을 사용하여 작업 목록을 바코드가 있는 PDF로 내보내기
type: docs
weight: 40
url: /ko/sharepoint/export-task-list-to-pdf-with-barcode-using-pdf-template-engine/
lastmod: "2020-12-16"
description: PDF SharePoint API는 PDF 템플릿 엔진을 사용하여 작업 목록을 바코드가 있는 PDF로 내보낼 수 있습니다.
---

{{% alert color="primary" %}}

이 문서에서는 SharePoint용 Aspose.PDF를 사용하여 작업 목록을 설정하고 바코드가 있는 PDF로 내보내는 방법을 보여줍니다.

{{% /alert %}}

템플릿 엔진을 사용하여 작업 목록을 바코드와 함께 PDF로 내보내려면 다음 단계를 수행하십시오.

1. Create and upload a template.
1. 템플릿 필드를 완성하고 템플릿을 저장합니다.
1. 새 작업을 만들고 저장합니다.
1. 문서를 PDF로 내보냅니다.

그 과정은 아래에 자세히 나와 있습니다.

## 작업 목록을 PDF로 내보내기

{{% alert color="primary" %}}

1. PDF 템플릿 목록을 만듭니다.

![Create PDF Template List](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_1.png)

2. 템플릿을 생성한 후 목록에서 **새 항목 추가**를 클릭하고 XML 파일을 업로드하세요.

![Upload XML File](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_2.png)

3. When uploaded is complete, click **OK**.
4. 양식 필드를 작성합니다.
5. 템플릿을 저장합니다.

![Configure Template](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_3.png)

템플릿이 구성되었습니다.

6. Go to the **Tasks** list and create a new task.
7. 작업을 저장합니다.

![Create Task](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_4.png)

8. **Aspose 도구** 탭에서 **PDF로 내보내기**를 클릭합니다.

![Export To PDF](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_5.png)

9. 구성된 템플릿을 선택하고 **내보내기**를 클릭합니다.

![Select Template](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_6.png)

The exported PDF:

![Exported PDF](export-task-list-to-pdf-with-barcode-using-pdf-template-engine_7.png)

{{% /alert %}}

