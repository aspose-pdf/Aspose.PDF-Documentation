...
Aspose.PDF for SharePoint 내보내기 기능을 사용하려면 먼저 “PDF 템플릿”을 사용하는 목록을 생성합니다.
...
- **Name**: 템플릿의 파일 이름.
- **Title**: 템플릿의 제목. (기본적으로 파일 이름과 동일합니다.)
- **Description**: 템플릿에 대한 설명. 좋은 설명은 템플릿을 더 쉽게 사용할 수 있게 합니다.
- **할당된 리스트 유형**: 콤마로 구분된 리스트 ID (템플릿과 관련됨. 이 필드는 **AllListTypes** 값을 포함할 수도 있습니다. 이 필드는 **Type** 필드가 **List** 로 설정된 경우에만 적용됩니다).
- **할당된 콘텐츠 유형**: 콤마로 구분된 콘텐츠 유형 ID (템플릿과 관련됨. 이 필드는 **AllListTypes** 로 설정될 수 있습니다. 이 필드는 **Type** 필드가 **Item** 로 설정된 경우에만 적용됩니다).
- **Type**: 리스트 템플릿 또는 아이템 템플릿 중 하나.
- **Status**: 옵션은 활성, 비활성 (모두에게 보이지 않음), 디버깅 (관리자에게만 표시) 입니다.
...
저장되면 새 템플릿이 템플릿 목록에 나타나며, 사용할 준비가 됩니다:
...
- **SPTableIterator** – 첫 번째 표 셀에 배치하고 전체 반복을 위해 표를 표시합니다.
- **SPRowIterator** – 첫 번째 표 셀에 배치하고 행 반복을 위해 표를 표시합니다.
- **SPField** – 항목 필드의 값으로 대체됩니다.

참고로, 다운로드해 주세요. [템플릿 XML 파일](attachments/8421394/8618082.zip).
#### **PDF로 내보내기**
