...
weight: 20
url: /ko/net/actions/
description: 이 섹션에서는 C#으로 프로그래밍 방식으로 문서 및 양식 필드에 작업을 추가하는 방법을 설명합니다.
lastmod: "2022-02-17"
sitemap:
    changefreq: "weekly"
    priority: 0.7
...
<script type="application/ld+json">
{
    ...
    "description": "이 섹션에서는 C#을 사용하여 문서 및 양식 필드에 동작을 프로그래밍 방식으로 추가하는 방법을 설명합니다."
}
</script>
...
1. Page와 [Rectangle](https://reference.aspose.com/pdf/ko/net/aspose.pdf/rectangle) 객체를 사용하여 [LinkAnnotation](https://reference.aspose.com/pdf/ko/net/aspose.pdf.annotations/linkannotation) 객체를 생성합니다. 사각형 객체는 링크가 추가될 페이지의 위치를 지정하는 데 사용됩니다.
...
- [FreeTextAnnotation](https://reference.aspose.com/pdf/ko/net/aspose.pdf.annotations/freetextannotation) 객체를 인스턴스화합니다. 이는 Page와 Rectangle 객체를 인수로 받아들이므로 LinkAnnotation 생성자에 지정된 것과 동일한 값을 제공할 수 있습니다.
...
또 다른 일반적인 시나리오는 TextFragmentAbsorber를 사용하여 문서에서 주어진 텍스트를 찾고 해당 영역을 사이트에 대한 하이퍼링크로 설정하는 것입니다. 이를 구현한 코드 조각은 아래와 같습니다.
...
## 동일한 PDF 내 페이지에 대한 하이퍼링크 생성

Aspose.PDF for .NET는 PDF 생성 및 조작에 대한 훌륭한 기능을 제공합니다. 또한 PDF 페이지에 링크를 추가하는 기능을 제공하며, 링크는 다른 PDF 파일의 페이지, 웹 URL, 애플리케이션 실행 또는 동일한 PDF 파일의 페이지로 연결될 수 있습니다. 로컬 하이퍼링크(동일한 PDF 파일 내 페이지에 대한 링크)를 추가하기 위해 [LocalHyperlink](https://reference.aspose.com/pdf/ko/net/aspose.pdf/localhyperlink) 클래스가 Aspose.PDF 네임스페이스에 추가되었으며, 이 클래스에는 하이퍼링크의 대상/목적지 페이지를 지정하는 데 사용되는 TargetPageNumber 속성이 있습니다.

로컬 하이퍼링크를 추가하려면 TextFragment를 생성하여 링크를 TextFragment와 연결해야 합니다. [TextFragment](https://reference.aspose.com/pdf/ko/net/aspose.pdf.text/textfragment) 클래스에는 LocalHyperlink 인스턴스를 연결하는 데 사용되는 Hyperlink 속성이 있습니다. 다음 코드 조각은 이 요구 사항을 충족하는 단계를 보여줍니다.
...
## PDF 하이퍼링크 목적지(URL) 가져오기

링크는 PDF 파일에서 주석으로 표현되며 추가, 업데이트 또는 삭제할 수 있습니다. Aspose.PDF for .NET는 또한 PDF 파일에서 하이퍼링크의 목적지(URL)를 가져오는 기능을 지원합니다.
...
1. [AnnotationSelector](https://reference.aspose.com/pdf/ko/net/aspose.pdf.annotations/annotationselector) 객체의 Selected 속성을 사용하여 선택된 모든 링크 주석을 IList 객체로 가져옵니다.
1. 마지막으로, LinkAnnotation Action을 GoToURIAction으로 추출합니다.

다음 코드 조각은 PDF 파일에서 하이퍼링크 목적지(URL)를 가져오는 방법을 보여줍니다.
...
하이퍼링크에는 문서에 표시되는 텍스트와 목적지 URL 두 부분이 있습니다. 어떤 경우에는 URL이 아닌 텍스트가 필요할 수 있습니다.

PDF 파일의 텍스트와 주석/작업은 다른 엔티티로 표현됩니다. 페이지의 텍스트는 단어와 문자 집합일 뿐이며, 주석은 하이퍼링크와 같은 상호 작용성을 제공합니다.

URL 콘텐츠를 찾으려면 주석과 텍스트 모두 작업해야 합니다. [Annotation](https://reference.aspose.com/pdf/ko/net/aspose.pdf.annotations/annotation) 객체 자체에는 텍스트가 없지만 페이지의 텍스트 아래에 위치합니다. 따라서 텍스트를 가져오려면 주석은 URL의 경계를 제공하고, 텍스트 객체는 URL 콘텐츠를 제공합니다. 다음 코드 조각을 참조하십시오.
...
[문서를 볼 때 PDF 페이지를 지정하는 방법](#how-to-specify-pdf-page-when-viewing-document)에서는 문서가 첫 페이지가 아닌 다른 페이지에서 열리도록 지정하는 방법을 설명했습니다. 여러 문서를 결합할 때 하나 이상의 문서에 GoTo 작업이 설정되어 있으면 이를 제거하고 싶을 수 있습니다. 예를 들어, 두 문서를 결합하고 두 번째 문서에 두 번째 페이지로 이동하는 GoTo 작업이 있는 경우, 출력 문서는 결합된 문서의 첫 페이지가 아닌 두 번째 문서의 두 번째 페이지에서 열립니다. 이 동작을 방지하려면 열기 작업 명령을 제거하십시오.
...
다음 코드 조각은 PDF 파일에서 문서 열기 작업을 제거하는 방법을 보여줍니다.
...
## 문서를 볼 때 PDF 페이지를 지정하는 방법 {#how-to-specify-pdf-page-when-viewing-document}

Adobe Reader와 같은 PDF 뷰어에서 PDF 파일을 볼 때 일반적으로 파일은 첫 페이지에서 열립니다. 그러나 파일이 다른 페이지에서 열리도록 설정할 수 있습니다.

[XYZExplicitDestination](https://reference.aspose.com/pdf/ko/net/aspose.pdf.annotations/xyzexplicitdestination) 클래스를 사용하면 열고자 하는 PDF 파일의 페이지를 지정할 수 있습니다. GoToAction 객체 값을 [Document](https://reference.aspose.com/pdf/ko/net/aspose.pdf/document) 클래스의 OpenAction 속성에 전달하면 문서는 XYZExplicitDestination 객체에 대해 지정된 페이지에서 열립니다. 다음 코드 조각은 문서 열기 작업으로 페이지를 지정하는 방법을 보여줍니다.
