...
### Замена текст на всех страницах PDF‑документа

{{% alert color="primary" %}}

Вы можете попытаться найти и заменить текст в документе с помощью Aspose.PDF и получить результаты онлайн [здесь](https://products.aspose.app/pdf/redaction)
...
### Замена текст в определённой области страницы
...
1. Замените найденный текст. Каждое вхождение 'doc' в определённой области становится 'DOC'.
1. Сохраните обновленный PDF.
...
### Изменение размер и сдвинуть текст без изменения размера шрифта
...
### Изменение размер и сдвинуть абзац в PDF
...
1. Выберите фрагмент для изменения.
1. Измените размер и сместить абзац. Использовать медиакоробку страницы для определения границ и корректировать прямоугольник.
1. Настройте интервал. Это изменяет интервал между словами/буквами вместо изменения размера шрифта.
...
### Замена текст и автоматически расширить шрифт, чтобы заполнить целевую область
...
### Замена текст и вписать его в прямоугольник
...
1. Переберите найденные фрагменты текста.
...
### Замена текст на основе регулярного выражения
...
## Замена шрифты или удалить неиспользуемые шрифты

### Замена шрифты в существующем PDF файле
...
1. Определите и замените шрифты. Скрипт проверяет, является ли текущий шрифт фрагмента 'Arial-BoldMT'. Если да, он заменяет его шрифтом 'Verdana' с помощью метода FontRepository.find_font().
1. Сохраните изменённый документ.
...
### Удаление неиспользуемые шрифты
...
1. Удалите всё текстовое содержимое. Метод ‘absorber.remove_all_text()’ удаляет каждый текстовый элемент из загруженного документа, оставляя нетекстовые компоненты нетронутыми.
1. Сохраните обновлённый документ.
...
### Удаление весь текст с конкретной страницы
...
### Удаление весь текст из определённой области на странице PDF
...
### Удаление весь скрытый текст из PDF‑документа
