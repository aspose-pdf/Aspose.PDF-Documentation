...
В этом примере метод pdf.PageSetSize() изменяет размер первой страницы PDF‑документа. Константа `PageSize::A1` гарантирует, что первая страница будет установлена на размер бумаги A1. Это полезно при конвертации документов в стандартизированный формат или при обеспечении того, чтобы конкретный контент правильно помещался на страницах.

1. Откройте PDF‑документ с помощью метода [open](https://reference.aspose.com/pdf/rust-cpp/core/open/).
1. Установите размер страницы с помощью функции [page_set_size](https://reference.aspose.com/pdf/rust-cpp/organize/page_set_size/).
1. Сохраните документ с помощью метода [save_as](https://reference.aspose.com/pdf/rust-cpp/core/save_as/).
