---
title: Извлечение таблицы из PDF
...
Может показаться, что извлечение любых данных из PDF-документа является довольно сложной задачей. Однако библиотека **Aspose.PDF для C++** позволяет вам справиться с этой задачей. Извлекайте таблицы из вашего PDF-файла с помощью C++:
...
Следующий фрагмент кода показывает шаги по извлечению границы таблицы как изображения из PDF-документа:
...
void ExtractTableBorderAsImage()
{
    ...
    // Значение ctm матрицы по умолчанию 1,0,0,1,0,0
    auto lastCTM = MakeObject<System::Drawing::Drawing2D::Matrix>(1, 0, 0, -1, 0, 0);
    // Система координат System.Drawing основана на верхнем левом угле, а система координат pdf основана на нижнем левом угле, поэтому необходимо применить матрицу инверсии
