...
Мы улучшили как производительность, так и потребление памяти при конвертации PDF с большим количеством изображений. Скорость обработки теперь в два раза быстрее, а потребление памяти уменьшено на 10% в протестированных сценариях.
...
Возможность передачи пути к внешнему ICC профилю для конвертации PDF/X и PDF/A уже существовала в библиотеке в течение нескольких лет, активируемая свойством PdfFormatConversionOptions.IccProfileFileName. Теперь также возможно передавать данные для заполнения свойств OutputIntent с использованием объекта класса OutputIntent.

Следующий фрагмент кода показывает, как конвертировать аннотированный документ в PDF/X-1 с использованием ICC профиля FOGRA39:
```csharp
...
Начиная с версии 24.11, мы добавили возможность выбора алгоритма хеширования для Pkcs7Detached. По умолчанию используется SHA-256. Для цифровых подписей ECDSA алгоритм дайджеста по умолчанию зависит от длины ключа.
...
Начиная с версии 24.9, стало возможным генерировать отчет о сбое, когда библиотека выбрасывает исключение. Отчет о сбое включает информацию о типе исключения, названии приложения, версии Aspose.PDF, версии ОС, сообщении об ошибке и трассировке стека.
...
Извлечение элементов слоя документа PDF и сохранение в новый поток PDF теперь доступно. В PDF документах слои (также известные как группы опционального содержимого или OCG) используются для различных целей, в первую очередь для управления и контроля видимости содержимого внутри документа. Эта функциональность особенно полезна в дизайне, инженерии и издательском деле. Например: аспекты чертежей, компоненты сложных диаграмм, языковые версии одного и того же содержимого.
...
Чтобы конвертировать изображения HEIC в PDF, пользователю необходимо добавить ссылку на пакет NuGet `FileFormat.HEIC` и использовать следующий фрагмент кода:
...
Начиная с версии 24.8, стало возможным конвертировать PDF документы в PDF/A-4. Часть 4 стандарта, основанная на PDF 2.0, была опубликована в конце 2020 года.
...
С 24.8 мы представили метод для упрощения прозрачного содержимого в PDF документах:
...
С момента релиза 24.6, в рамках редактирования помеченного PDF, были добавлены методы в **Aspose.Pdf.LogicalStructure.Element**:

- Тег (добавление тегов к конкретным операторам, таким как изображения, текст и ссылки)
- InsertChild
- RemoveChild
- ClearChilds

Также в этом релизе возможно создание доступного PDF с использованием низкоуровневых функций:
...
Библиотека Aspose.PDF for .NET открывает PDF, проходит через каждый слой на первой странице и упрощает каждый слой, делая его постоянным на странице.
...
Метод 'Layer.Flatten(bool cleanupContentStream)' принимает булевый параметр, который указывает, следует ли удалять маркеры опциональной группы содержимого из потока содержимого. Установка параметра cleanupContentStream в false ускоряет процесс упрощения.
...
С 24.4 вы можете выбрать источник бумаги по размеру страницы PDF в диалоговом окне печати, используя API

Начиная с Aspose.PDF 24.4, этот параметр можно включать и выключать с помощью свойства Document.PickTrayByPdfSize или фасада PdfContentEditor:
...
С 24.3 стало возможным добавить пустое поле подписи на каждой странице в файл PDF/A
...
Текущее обновление представляет три версии удаления тегов из помеченных PDF.
...
Обратите внимание, что измерение основано на шрифте, встроенном в документ. Если какая-либо информация для измерения отсутствует, этот метод возвращает 0.
...
Функция для обнаружения инкрементальных обновлений в PDF документе была добавлена. Эта функция возвращает 'true', если документ был сохранен с инкрементальными обновлениями; в противном случае она возвращает 'false'.
...
С Aspose.PDF 23.7 поддержка добавления извлечения формы:
...
Метки принтера — это графические символы или текст, добавленные на страницу, чтобы помочь производственному персоналу идентифицировать компоненты многопластовой работы и поддерживать согласованный вывод в процессе производства. Примеры, обычно используемые в полиграфической отрасли, включают:

- Цели регистрации для выравнивания пластин
- Серые градиенты и цветовые полосы для измерения цветов и плотности чернил
- Разметки, показывающие, где выходной носитель должен быть обрезан
...
```
Также поддержка извлечения векторных изображений. Попробуйте использовать следующий код для обнаружения и извлечения векторной графики:
...
- Удаление UR подписей при сохранении документа;
...
- Форматирование поля как числа и формата даты;
...
Настройка цвета фона для внешнего вида подписи и цвета шрифта меток в области подписи с Aspose.PDF for .NET.
...
С Aspose.PDF for .NET вы можете скрывать изображения, используя ImagePlacementAbsorber из документа:
...
С помощью следующего фрагмента кода вы сможете получить доступ к зашифрованному содержимому ваших PDF файлов, защищенных Azure Information Protection:
...
### После конвертации в HTML шрифт полностью встроен в выходной файл
