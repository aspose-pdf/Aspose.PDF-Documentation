...
description: Узнайте, как добавить всплывающую подсказку к текстовому фрагменту в PDF с использованием C# и Aspose.PDF
lastmod: "2022-02-17"
sitemap:
    changefreq: "weekly"
    priority: 0.7
...
<script type="application/ld+json">
{
    ...
    "abstract": "Улучшите свои PDF-документы с помощью новой функции PDF Tooltip, используя C#. Эта функциональность позволяет без проблем добавлять всплывающие подсказки к текстовым фрагментам в PDF-файлах, предоставляя пользователям дополнительную информацию при наведении курсора. Используйте невидимые кнопки и скрытые текстовые блоки, чтобы создать динамичный и интерактивный опыт чтения с Aspose.PDF",
    "author": {
        "@type": "Person",
...
## Добавление всплывающей подсказки к искомому тексту с помощью невидимой кнопки

Часто требуется добавить некоторые детали к фразе или конкретному слову в качестве всплывающей подсказки в PDF-документе, чтобы она появлялась, когда пользователь наводит курсор мыши на текст. Aspose.PDF for .NET предоставляет эту функцию для создания всплывающих подсказок, добавляя невидимую кнопку поверх искомого текста. Следующий фрагмент кода покажет вам, как достичь этой функциональности:
...
Что касается длины всплывающей подсказки, текст всплывающей подсказки содержится в PDF-документе как строка типа PDF, вне потока содержимого. Нет эффективного ограничения на такие строки в PDF-файлах (см. Приложение C к справочнику PDF). Однако соответствующий читатель (например, Adobe Acrobat), работающий на определенном процессоре и в определенной операционной среде, действительно имеет такое ограничение. Пожалуйста, обратитесь к документации вашего приложения для чтения PDF.

{{% /alert %}}

## Создание скрытого текстового блока и его отображение при наведении мыши

В Aspose.PDF реализована функция скрытия действий, с помощью которой можно показывать/скрывать текстовое поле (или любой другой тип аннотации) при наведении/выходе мыши над некоторой невидимой кнопкой. Для этой цели используется класс Aspose.Pdf.Annotations.HideAction для назначения действия скрытия/показа текстового блока. Пожалуйста, используйте следующий фрагмент кода, чтобы показать/скрыть текстовый блок при наведении/выходе мыши.

Также учитывайте, что действия PDF в документах хорошо работают в соответствующих читателях (например, Adobe Reader), но нет гарантий для других PDF-ридеров (например, плагинов веб-браузеров). Мы провели краткое исследование и обнаружили:

- Все реализации действия скрытия в PDF-документах хорошо работают в Internet Explorer v.11.0.
- Все реализации действия скрытия также работают в Opera v.12.14, но мы заметили некоторую задержку ответа при первом открытии документа.
- Только реализация, использующая конструктор HideAction, принимающий имя поля, работает, если документ просматривается в Google Chrome v.61.0; пожалуйста, используйте соответствующие конструкторы, если просмотр в Google Chrome имеет значение:
