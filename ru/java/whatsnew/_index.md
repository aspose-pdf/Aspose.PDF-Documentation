...
weight: 10
url: /java/whatsnew/
description: На этой странице представлены самые популярные новые функции Aspose.PDF for Java, которые были добавлены в последних выпусках.
sitemap:
    changefreq: "monthly"
    priority: 0.8
lastmod: "2026-06-09"
...
### Улучшения очистки скрытых данных

Улучшенная очистка PDF теперь доступна через HiddenDataSanitizer для более эффективного удаления скрытого контента из документов.
...
### Поддержка конвертации PDF в PDF/E

Aspose.PDF for Java теперь поддерживает конвертацию PDF‑документов в формат PDF/E.
...
Добавлена поддержка вставки HTML‑текста внутри аннотаций.
...
### Поддержка соответствия PDF 1.6

Добавлена поддержка соответствия PDF 1.6 для сценариев, требующих этой версии документа.

## Что нового в Aspose.PDF 25.8

### Поддержка стилей границ таблиц

Добавлена поддержка стилей границ таблиц, предоставляющая больший контроль над внешним видом таблиц.
...
### Извлечение ALT‑текста для изображений в PDF

Теперь можно получать описания ALT‑текста для изображений в PDF‑документах, что помогает при обработке, ориентированной на доступность.
...
### Улучшенное форматирование вывода PDF в DOCX

Конвертация PDF в DOCX улучшена для документов, у которых ранее формат вывода был некорректным.
...
### Сохранение изображений при конвертации PDF в ODS

Изображения теперь сохраняются при конвертации PDF‑документов в ODS.
...
### Автоматическое создание тегов при конвертации PDF в PDF/A

Конвертация PDF в PDF/A теперь поддерживает автоматическое создание тегов для улучшения результатов разметки в выходном документе.
...
### Сохранение гиперссылок при конвертации PDF в XLSX

Гиперссылки теперь сохраняются при конвертации PDF‑документов в XLSX, улучшая навигацию в экспортированных таблицах.
...
Начиная с версии 25.2, была добавлена возможность обнаружения компрометации цифровых подписей PDF. Вы можете использовать класс `SignaturesCompromiseDetector` для проверки цифровых подписей на компрометацию. Вызовите метод `check()` для проверки подписей документа. Если компрометация подписей не обнаружена, метод вернёт `true`. Чтобы проверить, покрывают ли существующие подписи весь документ, используйте свойство `SignaturesCoverage`.
...
Начиная с версии 25.2 добавлена возможность конвертировать PDF в формат файла PDF/X-4:
...
Начиная с версии 25.2 стало возможным выравнивать вывод HTML по центру:
...
Также, начиная с версии 25.2, стало возможным получать высоту подъёма (ascent) и спуска (descent) текста для заданного шрифта и размера с помощью Aspose.PDF. Новая возможность реализована в классе `com.aspose.pdf.Font`.

Добавлены методы:

**Измеряет максимальную точку подъёма**

- public double getAscentPoint(String str, float fontSize)

**Измеряет максимальную точку спуска**
...
Возможность передавать путь к внешнему ICC‑профилю для конвертации PDF/X и PDF/A уже существовала в библиотеке несколько лет, реализованная через свойство `PdfFormatConversionOptions.IccProfileFileName`. Теперь также можно передавать данные для заполнения свойств `OutputIntent` с помощью объекта класса `OutputIntent`.

Следующий фрагмент кода показывает, как конвертировать документ с аннотациями в PDF/X-1, используя ICC‑профиль аннотации FOGRA39:
...
Начиная с версии 25.1 добавлена возможность получать информацию о привилегиях при работе с документами:
...
Начиная с версии 24.12, стало возможным конвертировать PDF‑документы в PDF/A-4. Часть 4 стандарта, основанная на PDF 2.0, была опубликована в конце 2020 года.
...
В этом выпуске возможно создать доступный PDF, используя низкоуровневые функции:

Следующий фрагмент кода работает с PDF‑документом и его тегированным содержимым, используя библиотеку Aspose.PDF для обработки.
...
Класс `GraphicalPdfComparer` добавлен для графического сравнения PDF‑документов и страниц. Графическое сравнение работает с изображениями страниц документа. Он возвращает результат в виде объекта `ImagesDifference` или в виде PDF‑документа, содержащего изображения, объединённые из оригинала и различий. Графическое сравнение наиболее полезно для документов, в которых есть небольшие различия в тексте или графическом содержимом.
...
Также возможно добавить альтернативный текст для Image Stamp:

Свойство AlternativeText было добавлено в ImageStamp — если ему присвоить значение, то при добавлении ImageStamp в документ будет установлен альтернативный текст.
...
Также следующий код показывает, как добавить AlternativeText к существующим изображениям в FigureElements.
...
С версии 24.7, в рамках редактирования помечённого PDF, были добавлены методы в **Aspose.Pdf.LogicalStructure.Element**:

- Tag (добавлять теги к определённым операторам, таким как изображения, текст и ссылки)
- InsertChild
- RemoveChild
- ClearChilds

Эти методы позволяют редактировать теги PDF‑файла, например:
...
Начиная с версии 24.6, Aspose.PDF для Java позволяет подписывать PDF с помощью `java.security.cert.X509Certificate` и `java.security.PrivateKey`:
...
В этой версии мы получили возможность работать со слоями PDF. Например:

- заблокировать слой PDF
- извлечь элементы слоя PDF
- упростить (flatten) многоуровневый PDF
- объединить все слои внутри PDF в один

**Блокировка слоя PDF**

Начиная с версии 24.5 вы можете открыть PDF, заблокировать определённый слой на первой странице и сохранить документ с изменениями. Добавлены два новых метода и одно свойство:
...
Библиотека Aspose.PDF for Java позволяет извлекать каждый слой с первой страницы и сохранять каждый слой в отдельный файл.
...
**Упрощение (flatten) многоуровневого PDF**

Библиотека Aspose.PDF for Java открывает PDF, перебирает каждый слой на первой странице и упрощает (flatten) каждый слой, делая его постоянным на странице.
...
Метод Layer.flatten(boolean cleanupContentStream) принимает булевый параметр, указывающий, следует ли удалять маркеры группы необязательного контента из потока содержимого.  
Установка параметра cleanupContentStream в false ускоряет процесс упрощения.

**Объединение всех слоёв внутри PDF в один**

Библиотека Aspose.PDF for Java позволяет объединять либо все слои PDF, либо конкретный слой на первой странице в новый слой и сохранять обновлённый документ.

Для объединения всех слоёв на странице добавлены два метода:

- `void mergeLayers(String newLayerName);`
- `void mergeLayers(String newLayerName, String newOptionalContentGroupId);`

Второй параметр позволяет переименовать маркер группы необязательного контента. Значение по умолчанию — "oc1" (/OC /oc1 BDC).
...
В этой версии были представлены Java‑плагины для PDF:

- **Плагин Form Flattener**
...
Начиная с версии 24.2 возможно добавить водяной знак в PDF с помощью AcroForms. TextStamp подходит для использования с файлами AcroForm. Если использовать TextStamp для файлов XFA, текст будет отрисован на странице как в обычном PDF‑файле (его можно увидеть в тех PDF‑просмотрщиках, которые не умеют читать XFA, например, в браузере Chrome). Чтобы добавить текст в файл XFA, его необходимо изменить во внутреннем XML‑файле XFA.
...
### Установка StateModel для аннотации

Мы можем использовать методы `setReviewState` и `setMarkedState` из класса `MarkupAnnotation` для установки необходимого состояния. У всех разметочных аннотаций доступна опция **Set State**.
...
Начиная с версии 24.2 реализовано преобразование OFD в PDF:
...
В версии 24.1 реализовано преобразование PDF в Markdown:
...
Форма может быть найдена, и текст может быть заменён с помощью следующего фрагмента кода:
...
Начиная с этой версии возможно удалить скрытый текст из PDF‑файла:
...
Текущее обновление представляет три варианта удаления тегов из помеченных PDF.

- Удалите некоторый элемент узла из `documentElement` (корневого элемента дерева):
...
Мы реализовали новую функцию измерения высоты символа. Используйте следующий код для измерения высоты символа:
...
Добавление изображения с `ImageFilterType.Flate` не сохраняет прозрачность.
...
В версии 23.8 добавлена функция обнаружения инкрементных обновлений в PDF‑документе. Эта функция возвращает `true`, если документ был сохранён с инкрементными обновлениями, иначе возвращает `false`.
...
Ещё одна функция — копирование `OutputIntents` из входного PDF в целевой PDF.

Мы добавили новое публичное свойство `Document.getOutputIntents()` для доступа к output intents в документе. Пока поддерживается только использование уже существующих в некоторых документах output intents, пользователь не может создать `OutputIntent` с нуля.
...
Начиная с Aspose.PDF 23.8 поддерживается извлечение фигур:
...
Начиная с версии 23.7 поддерживается масштабирование страниц в диалоговом окне печати (Print Dialog Presets Page Scaling):
...
Начиная с версии 23.6 поддерживается API для позиционирования векторной графики:
...
Начиная с версии 23.1 поддерживается создание аннотации `PrinterMark`. Добавлен один из вариантов аннотации: `ColorBarAnnotation`.
...
Начиная с этой версии поддерживается конвертация PDF в DICOM‑изображение:
...
Начиная с версии 22.09 поддерживается добавление свойства для изменения порядка рубрик субъекта (E=, CN=, O=, OU=) в подписи.
...
Начиная с Aspose.PDF 23.8 поддерживается добавление метода для восстановления таблицы xref:
...
PDF в PDF_A_1A — реализована опция удаления цвета прозрачности, чтобы избежать большого размера выходного файла.

Начиная с версии 22.5 клиент может управлять качеством преобразованной прозрачности, а также размером выходного файла в результате:
...
Во время конвертации в PDF/A прозрачный контент удаляется и заменяется изображением.  
Мы реализовали новую функцию, и теперь клиент может управлять качеством изображения с помощью параметра TransparencyResolution:
...
Этот релиз включает информацию для Aspose.PDF for Java:
...
Этот релиз включает PDF в XSLX: поддержка RTL (иврит, арабский).
...
Также вы можете объединять изображения в формате Tiff:
...
Начиная с версии 24.2 можно добавлять водяной знак в PDF с помощью AcroForms. `TextStamp` подходит для использования с файлами AcroForm. Если использовать `TextStamp` для XFA‑файлов, текст рисуется на странице как в обычном PDF‑файле (его можно увидеть в тех PDF‑просмотрщиках, которые не умеют читать XFA‑файлы, например, в браузере Chrome). Чтобы добавить текст в XFA‑файл, его необходимо изменить во внутреннем XML‑файле XFA.
...
Мы можем использовать `setReviewState` и `setMarkedState` из класса `MarkupAnnotation`, чтобы задать необходимое состояние. У всех разметочных аннотаций доступна опция **Set State**.
...
Также в версии 24.1 реализовано прерывание потока с использованием InterruptMonitor.
...
Форма может быть найдена, и её текст может быть заменён с помощью следующего фрагмента кода:
...
Начиная с этой версии можно удалять скрытый текст из PDF‑файла:
...
Текущее обновление представляет три варианта удаления тегов из помеченных PDF‑документов.
...
Мы реализовали новую возможность измерения высоты символа. Используйте следующий код, чтобы измерить высоту символа:
...
Функция для обнаружения инкрементных обновлений в PDF‑документе была добавлена в версии 23.8. Эта функция возвращает `true`, если документ был сохранён с инкрементными обновлениями, в противном случае возвращает `false`.
...
Мы добавляем новое публичное свойство `Document.getOutputIntents()`, позволяющее получить доступ к OutputIntents в документе. На данный момент поддерживается только использование уже существующих в документе OutputIntents; пользователь не может создать OutputIntent с нуля.
...
Начиная с версии 23.6, поддерживается возможность задавать заголовок страницы HTML и EPUB.
...
В этой версии добавлена поддержка преобразования PDF в изображение DICOM:
...
Начиная с Aspose.PDF 23.8 поддерживается метод для восстановления таблицы xref:
...
Начиная с версии 22.5 клиент может управлять качеством преобразованной прозрачности, а также размером выходного файла:
...
Во время конвертации в PDF/A прозрачный контент удаляется и заменяется изображением.  
Мы реализовали новую функцию, и теперь пользователь может управлять качеством изображения с помощью параметра **TransparencyResolution**:
...
Этот выпуск содержит информацию для Aspose.PDF for Java:
...
- PDF → XMLSpreadSheet2003: распознавание текста в нижнем и верхнем индексе;

- PDF → Excel: распознавание текста в нижнем и верхнем индексе;
...
Этот релиз включает PDF to XSLX: поддержка RTL (иврит, арабский).

## Что нового в Aspose.PDF 22.1

Aspose.PDF for Java позволяет загружать документы формата Portable Document Format (PDF) версии 2.0.
