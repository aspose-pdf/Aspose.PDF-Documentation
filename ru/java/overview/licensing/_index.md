...
- **PDF создаётся с оценочным водяным знаком.** Оценочная версия Aspose.PDF for Java предоставляет полный функционал продукта, но все страницы в сгенерированных PDF‑документах помечены водяным знаком \"Evaluation Only. Created with Aspose.PDF. Copyright 2002-2020 Aspose Pty Ltd\" в верхней части.

- **Ограничение количества элементов коллекции, которые могут быть обработаны.**
В оценочной версии вы можете обработать только четыре элемента любой коллекции (например, только 4 страницы, 4 поля формы и т.д.).
...
>Если вы хотите протестировать Aspose.PDF for Java без ограничений оценочной версии, вы можете также запросить 30‑дневную временную лицензию. См. раздел [«Как получить временную лицензию?»](https://purchase.aspose.com/temporary-license)
...
Начиная с Aspose.PDF for Java 4.2.0, вам необходимо выполнить следующий код для инициализации лицензии.
...
Aspose.PDF позволяет разработчикам применять ключ с измерением. Это новый механизм лицензирования. Новый механизм лицензирования будет использоваться вместе с существующим методом лицензирования. Клиенты, которые хотят платить за использование функций API, могут использовать лицензирование с измерением. Подробнее см. в разделе [FAQ по лицензированию с измерением](https://purchase.aspose.com/faqs/licensing/metered).

Новый класс [Metered](https://reference.aspose.com/pdf/java/com.aspose.pdf/Metered) добавлен для применения метрированного ключа. Ниже приведён пример кода, демонстрирующего, как задать публичный и приватный метрированный ключ.
...
- **Установите лицензию для каждого продукта Aspose отдельно.** Даже если у вас один файл лицензии для всех компонентов, например ‘Aspose.Total.lic’, вызывайте **License.SetLicense** отдельно для каждого продукта Aspose, используемого в приложении.
- **Используйте полностью квалифицированное имя класса лицензии.** У каждого продукта Aspose есть класс **License** в собственном пространстве имён. Например, в Aspose.PDF это **com.aspose.pdf.License**, а в Aspose.Words — **com.aspose.words.License**. Полное имя класса позволяет избежать путаницы при применении лицензии к конкретному продукту.
