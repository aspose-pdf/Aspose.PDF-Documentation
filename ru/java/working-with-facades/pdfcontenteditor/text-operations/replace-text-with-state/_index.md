---
title: Замена текста с помощью состояния
linktitle: Замена текста с помощью состояния
type: docs
weight: 20
...
---
## Замена текста с пользовательским состоянием текста

1. Привяжите исходный PDF к фасаду `PdfContentEditor`.
2. Создайте и настройте `TextState` с требуемым цветом и размером шрифта.
3. Установите область замены текста на `ReplaceAll`.
4. Вызовите `replaceText(...)` с текстом поиска, текстом замены и настроенным объектом `TextState`.
