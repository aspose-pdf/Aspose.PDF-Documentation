---
title: Получить метаданные PDF
linktitle: Получить метаданные PDF
type: docs
weight: 20
url: /java/get-pdf-metadata/
description: Узнайте, как читать метаданные PDF на Java с помощью фасада PdfFileInfo.
lastmod: "2026-06-09"
draft: false
sitemap:
    changefreq: "weekly"
    priority: 0.7
TechArticle: true
AlternativeHeadline: Получение метаданных PDF с помощью Aspose.PDF для Java.
Abstract: Узнайте, как получить метаданные PDF с помощью Aspose.PDF для Java. В примере Java считываются стандартные поля, такие как тема, заголовок, ключевые слова, создатель, дата создания и дата изменения, а также флаги состояния файла и пользовательская запись метаданных `Reviewer`.
---
## Получите метаданные PDF

В этом примере считывается стандартная информация о документе, флаги состояния файла и пользовательский ключ метаданных.

### Шаги

1. Создайте объект `PdfFileInfo` для исходного PDF-файла.
2. Прочтите стандартные поля метаданных, такие как тема, заголовок, ключевые слова и создатель.
3. Проверьте флаги состояния файла, например, является ли файл действительным, зашифрованным, защищенным паролем или портфолио.
4. Прочитайте значение пользовательского метаданных с помощью `getMetaInfo`.
5. Закройте экземпляр `PdfFileInfo`.

### Пример Java

```java
public static void getPdfMetadata(Path inputFile) {
    PdfFileInfo pdfInfo = new PdfFileInfo(inputFile.toString());
    System.out.println("Subject: " + pdfInfo.getSubject());
    System.out.println("Title: " + pdfInfo.getTitle());
    System.out.println("Keywords: " + pdfInfo.getKeywords());
    System.out.println("Creator: " + pdfInfo.getCreator());
    System.out.println("Creation Date: " + pdfInfo.getCreationDate());
    System.out.println("Modification Date: " + pdfInfo.getModDate());
    System.out.println("Is Valid PDF: " + pdfInfo.isPdfFile());
    System.out.println("Is Encrypted: " + pdfInfo.isEncrypted());
    System.out.println("Has Open Password: " + pdfInfo.hasOpenPassword());
    System.out.println("Has Edit Password: " + pdfInfo.hasEditPassword());
    System.out.println("Is Portfolio: " + pdfInfo.hasCollection());
    String reviewer = pdfInfo.getMetaInfo("Reviewer");
    System.out.println("Reviewer: " + (reviewer == null || reviewer.isBlank() ? "No Reviewer metadata found." : reviewer));
    pdfInfo.close();
}
```
