...
## Извлечение текста со всех страниц с использованием TextAbsorber
...
1. Создайте `TextExtractionOptions` и `TextSearchOptions` с настройками области поиска.
1. Запустите `TextAbsorber` на всех страницах и выведите извлечённый текст.
...
## Извлечение текста с одной страницы с помощью TextAbsorber

Используйте этот пример, когда извлечение обычного текста должно быть ограничено одной страницей.

1. Откройте исходный документ PDF.
1. Настройте извлечение текста и параметры поиска для целевой области.
...
## Проверка всех текстовых фрагментов в документе
...
## Поиск одной фразы на определённой странице
...
## Продолжение последовательного поиска по страницам
...
1. Выполните поиск на первой странице и проверьте результаты.
1. Продолжите поиск на следующих страницах и просмотрите обновлённые совпадения.
...
## Поиск фразы внутри выбранного прямоугольника

Используйте этот пример, когда сопоставление фраз должно быть ограничено областью на одной странице.

1. Откройте исходный документ PDF.
1. Создайте `TextFragmentAbsorber` с целевой фразой и `TextSearchOptions` с прямоугольной областью поиска.
...
1. Откройте исходный документ PDF.
1. Создайте массив шаблонов регулярных выражений и передайте его в `TextFragmentAbsorber`.
1. Посетите документ и проверьте сгруппированные результаты поиска по регулярным выражениям.
...
## Поиск текста и его преобразование в гиперссылки
...
## Подсветка результатов поиска в отрисованных превью страниц
...
1. Выполните поиск на каждой странице с помощью `TextFragmentAbsorber` и отобразите страницу в поток изображения.
