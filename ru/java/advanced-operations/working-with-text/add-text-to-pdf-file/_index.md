---
title: Добавить текст в PDF в Java
linktitle: Добавить текст в PDF
type: docs
weight: 10
url: /java/add-text-to-pdf-file/
description: Узнайте, как добавлять текст, фрагменты HTML, списки, ссылки и пользовательские шрифты в документы PDF на Java.
lastmod: "2026-06-09"
sitemap:
    changefreq: "monthly"
    priority: 0.7
TechArticle: true
AlternativeHeadline: Добавляйте текст, ссылки, HTML и шрифты в файлы PDF с помощью Java.
Abstract: В этой статье объясняется, как добавлять и стилизовать текст в PDF-документах с помощью Aspose.PDF для Java. Он охватывает простую вставку текста, макет абзаца, гиперссылки, текст с письмом справа налево, стиль шрифта, прозрачность, границы, фрагменты HTML и LaTeX, градиентный текст и пользовательские шрифты, загружаемые из файлов или потоков.
---
Aspose.PDF для Java поддерживает вставку обычного текста, расширенный макет, стили, градиенты, HTML, LaTeX и пользовательские шрифты.

## Добавьте простой фрагмент текста

Используйте этот пример, когда короткую текстовую строку необходимо разместить в фиксированных координатах страницы.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте `TextFragment` и установите его положение.
...
Используйте этот пример, когда большой блок текста необходимо разместить внутри ограниченной области.

1. Создайте новый PDF-документ и добавьте страницу.
1. Загрузите исходный текст и настройте прямоугольник `TextParagraph` и режим переноса.
...
## Добавляйте абзацы с разными настройками отступов

Используйте этот пример, когда первая и последующие строки должны использовать разные правила отступов.

1. Создайте новый PDF-документ и добавьте страницу.
1. Подготовьте общий текстовый фрагмент и создайте несколько объектов `TextParagraph`.
...
## Вставка текста с переносом строки вручную

Используйте этот пример, когда один фрагмент текста должен содержать явную новую строку.
...
## Проверка обнаруженных разрывов строк

Используйте этот пример, если вам нужно просмотреть вывод уведомлений, связанных с макетом текста и переносом строк.

1. Создайте новый PDF-документ и включите ведение журнала уведомлений.
1. Добавьте на страницу несколько длинных текстовых фрагментов.
1. Просмотрите уведомления и сохраните документ.
...
## Динамическое измерение ширины текста

Используйте этот пример, когда необходимо измерить ширину символов и строк перед принятием решения о макете.

1. Разрешите целевой шрифт и создайте `TextState`.
1. Измеряйте символы и сравнивайте результаты API состояния шрифта и текста.
1. Выведите все несоответствия для проверки.
...
Используйте этот пример, когда одна часть текстового фрагмента должна вести себя как веб-ссылка.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте `TextFragment` с несколькими объектами `TextSegment`.
1. Назначьте гиперссылку и стиль целевому сегменту, а затем сохраните документ.
...
Используйте этот пример, когда документ должен отображать содержимое сценария справа налево с правильным выравниванием.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте `TextFragment` с целевым текстом RTL и настройте его шрифт и выравнивание.
...
## Добавляйте стилизованный текст и сегменты в виде формул.

Используйте этот пример, когда обычный текст и сегменты, подобные индексам, должны использовать разные текстовые состояния в одном выводе.

1. Создайте новый PDF-документ и добавьте страницу.
1. Постройте основной стилизованный фрагмент и скомпонуйте формулу с вспомогательными сегментами.
1. Добавьте на страницу оба фрагмента и сохраните документ.
...
Используйте этот пример, когда фрагмент текста должен явно использовать стиль подчеркивания.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте фрагмент текста, настройте его шрифт и состояние подчеркивания, а также установите его положение.
1. Добавьте его с помощью `TextBuilder` и сохраните результат.
...
## Добавьте прозрачный текст поверх цветной фигуры

Используйте этот пример, когда текст должен отображаться с прозрачностью над фоновым изображением.

1. Создайте новый PDF-документ и добавьте страницу.
1. Нарисуйте фоновую фигуру и создайте полупрозрачный фрагмент текста.
...
Используйте этот пример, когда доступный для поиска или скрытый текст должен присутствовать без видимой визуализации.

1. Создайте новый PDF-документ и добавьте страницу.
1. Добавьте видимый фрагмент текста и второй фрагмент с включенным невидимым флагом.
...
Используйте этот пример, когда текст должен быть нарисован вместе с ограничивающим его прямоугольником.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте стиль `TextFragment` и включите рисование границы текстового прямоугольника.
1. Добавьте его с помощью `TextBuilder` и сохраните PDF.
...
Используйте этот пример, когда текст должен использовать зачеркнутое форматирование.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте стилизованный фрагмент текста с включенным зачеркиванием.
1. Добавьте его на страницу и сохраните документ.
...
## Применение осевой градиентной заливки к тексту

Используйте этот пример, когда в тексте должна использоваться заливка линейным градиентом вместо сплошного цвета.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте фрагмент текста и назначьте осевой градиент его цвету переднего плана.
1. Добавьте его на страницу и сохраните PDF.
...
## Применение радиальной градиентной заливки к тексту

Используйте этот пример, когда в тексте должна использоваться заливка радиальным градиентом.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте фрагмент текста и присвойте его цвету переднего плана радиальный градиент.
1. Добавьте его на страницу и сохраните документ.
...
## Добавьте встроенный форматированный текст в стиле HTML.

Используйте этот пример, когда форматирование верхнего и нижнего индекса необходимо вставить через разметку HTML.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте `HtmlFragment` с необходимой встроенной разметкой.
1. Добавьте его на страницу и сохраните PDF.
...
Используйте этот пример, когда математический контент или контент в формате TeX должен отображаться внутри PDF-файла.

1. Создайте новый PDF-документ и добавьте страницу.
1. Создайте `TeXFragment` с необходимым выражением.
1. Добавьте его на страницу и сохраните документ.
...
## Добавьте расширенный фрагмент HTML

Используйте этот пример, когда страница должна отображать структурированное содержимое HTML, такое как заголовки, абзацы и ссылки.

1. Создайте новый PDF-документ и добавьте страницу.
1. Подготовьте строку содержимого HTML и создайте `HtmlFragment`.
1. Добавьте его на страницу и сохраните PDF.
...
## Добавьте фрагмент HTML с переопределенным текстовым состоянием.

Используйте этот пример, когда импортируемое HTML-содержимое должно наследовать контролируемые настройки шрифта и цвета.

1. Создайте новый PDF-документ и добавьте страницу.
1. Подготовьте HTML-содержимое и создайте `HtmlFragment`.
1. Назначьте пользовательский `TextState`, добавьте фрагмент и сохраните документ.
...
## Используйте собственный шрифт, загруженный из файла

Используйте этот пример, когда в тексте должен использоваться шрифт, загруженный непосредственно из пути к файлу шрифта.

1. Устраните путь к файлу пользовательского шрифта.
1. Создайте фрагмент текста и загрузите шрифт через `FontRepository.openFont`.
...
## Используйте собственный шрифт, загруженный из потока

Используйте этот пример, когда пользовательский шрифт необходимо открыть из потока и внедрить в PDF-файл.
