---
title: Работа с текстом в Jython
linktitle: Работа с текстом в Jython
type: docs
weight: 40
url: /ru/java/working-with-text-in-jython/
description: Исследуйте манипулирование текстом в PDF с помощью Jython и Aspose.PDF, упрощая работу с текстовым содержимым в документах.
lastmod: "2026-08-19"
---
Этот раздел включает следующие темы:

- [Добавить HTML-строку с использованием DOM в Jython](/pdf/ru/java/add-html-string-using-dom-in-jython/)
- [Добавить текст в существующий PDF‑файл в Jython](/pdf/ru/java/add-text-to-an-existing-pdf-file-in-jython/)
- [Извлечь текст со всех страниц PDF‑документа в Jython](/pdf/ru/java/extract-text-from-all-the-pages-of-a-pdf-document-in-jython/)

