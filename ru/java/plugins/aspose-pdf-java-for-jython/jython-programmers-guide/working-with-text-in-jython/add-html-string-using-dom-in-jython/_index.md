---
title: Добавление HTML‑строки с использованием DOM в Jython
linktitle: Добавление HTML‑строки с использованием DOM в Jython
type: docs
weight: 10
url: /ru/java/add-html-string-using-dom-in-jython/
description: Изучите, как добавить HTML‑контент в PDF‑документ с помощью DOM в Jython, используя Aspose.PDF для создания насыщенных документов.
lastmod: "2026-08-19"
---
## Aspose.PDF — Добавление HTML

Для проверки формата файла с использованием **Aspose.PDF Java for Jython**. Здесь вы можете увидеть пример кода.

Ошибка при отображении макроса 'code' : Указано недопустимое значение для параметра lang

## Загрузка работающего кода

Скачайте исполняемый код с любого из указанных ниже социальных кодирующих сайтов:

- [GitHub](https://github.com/aspose-pdf/Aspose.PDF-for-Java/releases)


