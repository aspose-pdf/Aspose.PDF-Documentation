...
Страницы PDF не хранят текст так, как документы текстовых процессоров, поэтому порядок извлечения зависит от потока содержимого страницы и её макета. Для извлечения текста из определённых областей, анализа геометрии, обработки многоколоночных макетов, аннотаций, выделенного текста или обнаружения надстрочного и подстрочного текста используйте соответствующие статьи об извлечении в этом разделе.

## Извлечение текста со всех страниц

Используйте `TextAbsorber`, чтобы собрать единый поток текста из всего документа и записать его в файл. Это самый простой вариант, когда вам нужен только читаемый текстовый контент и не нужны границы абзацев или координаты.
...
1. Вызовите `document.getPages().accept(textAbsorber)`, чтобы обработать каждую страницу [Page](https://reference.aspose.com/pdf/java/com.aspose.pdf/page/) поглотителем.
...
Примените поглотитель только к нужной странице. Страницы в коллекции страниц `Document` нумеруются с 1, поэтому `get_Item(1)` читает первую страницу.

1. Откройте исходный PDF в экземпляре [Document](https://reference.aspose.com/pdf/java/com.aspose.pdf/document/).
1. Создайте [TextAbsorber](https://reference.aspose.com/pdf/java/com.aspose.pdf/textabsorber/) для извлечения текста с одной страницы.
1. Вызовите `accept(textAbsorber)` для страницы [Страница](https://reference.aspose.com/pdf/java/com.aspose.pdf/page/), выбранной по номеру.
1. Запишите извлеченный буфер текста в выходной файл.
...
Используйте `ParagraphAbsorber`, когда вам нужна структурная группировка вместо одного простого текстового потока. Он возвращает разметку страниц с разделами, абзацами, строками и объектами `TextFragment`, что полезно, когда вывод должен сохранять логические блоки текста.
...
1. Переберите разметки страниц, разделы, абзацы, строки и объекты [TextFragment](https://reference.aspose.com/pdf/java/com.aspose.pdf/textfragment/), предоставляемые поглотителем.
