---
title: Галерея примеров отчетов
linktitle: Галерея примеров отчетов
type: docs
weight: 40
url: /ru/jasperreports/sample-reports-gallery/
description: Просмотрите примеры отчетов, созданных с помощью Aspose.PDF for JasperReports. Посмотрите, как это расширяет возможности экспорта PDF.
lastmod: "2026-08-31"
---

{{% alert color="primary" %}}

В этой галерее показаны отчеты в формате PDF, экспортированные Aspose.PDF for JasperReports.

{{% /alert %}}

Показанные ниже отчеты основаны на примерах данных, установленных вместе с JasperServer.

## Отчет по всем счетам

![All accounts report](sample-reports-gallery_1.png)

## Отчет о магазинах Supermart

![Supermart stores report](sample-reports-gallery_2.png)

## Стандартные карты Эгейский отчет

![Standard charts Aegean report](sample-reports-gallery_3.png)

## Стандартные карты Эгейский отчет

![Standard charts Aegean report](sample-reports-gallery_4.png)

## Стандартный отчет о привлекательных диаграммах

![Standard charts eye candy report](sample-reports-gallery_5.png)

## Стандартный отчет о привлекательных диаграммах

![Standard charts eye candy report](sample-reports-gallery_6.png)

## Стандартный отчет о привлекательных диаграммах

![Standard charts eye candy report](sample-reports-gallery_7.png)

## Стандартный отчет с диаграммами

![Standard charts report](sample-reports-gallery_8.png)

## Стандартный отчет с диаграммами

![Standard charts report](sample-reports-gallery_9.png)
