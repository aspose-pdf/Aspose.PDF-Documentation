...
1. Вызов `AsposePdf` как Promise и выполните операцию по извлечению информации. Получите объект в случае успеха.
...
1. Вызов `AsposePdf` в виде Promise и выполнить операцию по извлечению шрифтов. Получить объект при успешном выполнении.
...
1. Вызов `AsposePdf` как Promise и выполнить операцию. Получите объект, если успешно.
...
## Удалить информацию о PDF-файле
...
1. Удалить информацию о PDF-файле. Таким образом, если 'json.errorCode' равно 0, результат операции сохраняется в "ResultPdfRemoveMetadata.pdf". Если параметр json.errorCode не равен 0 и, соответственно, в вашем файле появляется ошибка, информация об ошибке будет содержаться в 'json.errorText'.
