...
lastmod: "2025-09-27"
description: Este tema le muestra cómo convertir PDF a formatos PDF/x usando Aspose.PDF para Python a través de .NET.
sitemap: 
    changefreq: "monthly"
    priority: 0.8
TechArticle: true
AlternativeHeadline: Cómo convertir PDF a formatos PDF/x
Abstract: El artículo ofrece una guía completa sobre cómo convertir PDF a formatos PDF/A, PDF/E y PDF/X usando Aspose.PDF para Python.
---
...
**Aspose.PDF para Python** le permite convertir un archivo PDF a un archivo PDF compatible con <abbr title="Portable Document Format / A">PDF/A</abbr>. Antes de hacerlo, el archivo debe ser validado. Este tema explica cómo.

{{% alert color="primary" %}}

Tenga en cuenta que seguimos Adobe Preflight para validar la conformidad PDF/A. Todas las herramientas del mercado tienen su propia “representación” de la conformidad PDF/A. Por favor, consulte este artículo sobre herramientas de validación PDF/A como referencia. Elegimos productos de Adobe para verificar cómo Aspose.PDF genera archivos PDF porque Adobe está en el centro de todo lo relacionado con PDF.

{{% /alert %}}

Convertir el archivo usando el método Convert de la clase Document. Antes de convertir el PDF a un archivo compatible con PDF/A, valide el PDF usando el método Validate. El resultado de la validación se almacena en un archivo XML y luego este resultado también se pasa al método Convert. También puede especificar la acción para los elementos que no pueden convertirse usando la enumeración ConvertErrorAction.

{{% alert color="success" %}}
**Intente convertir PDF a PDF/A en línea**

Aspose.PDF para Python le presenta una aplicación gratuita en línea ["PDF a PDF/A-1A"](https://products.aspose.app/pdf/conversion/pdf-to-pdfa1a), donde puede intentar investigar la funcionalidad y la calidad con la que funciona.

[![Conversión Aspose.PDF PDF a PDF/A con Aplicación Gratis](pdf_to_pdfa.png)](https://products.aspose.app/pdf/conversion/pdf-to-pdfa1a)
{{% /alert %}}

El método 'document.validate()' valida si un archivo PDF se ajusta al estándar PDF/A-1B (una versión estandarizada por ISO del PDF diseñada para archivo a largo plazo). Los resultados de la validación se guardan en un archivo de registro.
...
- Formato objetivo - 'ap.PdfFormat.PDF_A_1B' (estándar de archivo).
...
Este ejemplo demuestra cómo convertir un documento PDF a formatos estandarizados más recientes: PDF 2.0 y PDF/A-4.
Ambas conversiones ayudan a garantizar el cumplimiento de especificaciones modernas y requisitos de archivo.
...
- Ruta del archivo de registro para los detalles de la conversión.
- Formato objetivo - 'ap.PdfFormat.V_2_0'.
- Acción de error - 'ap.ConvertErrorAction.DELETE' para eliminar los elementos no compatibles.
1. Realice una segunda conversión a PDF/A-4 usando el mismo método, asegurando que el archivo también cumpla con los estándares de archivo.
...
### Convertir PDF a PDF/A-3A con Archivos Embebidos

El próximo fragmento de código demuestra cómo incrustar archivos externos en un PDF y luego convertir el PDF al formato PDF/A-3A, que soporta adjuntos y es adecuado para archivado a largo plazo con contenido embebido.

1. Cargue el PDF de entrada usando 'ap.Document'.
1. Crear un objeto 'FileSpecification' que apunte al archivo a incrustar (p.ej., "aspose-logo.jpg") con una descripción.
1. Añadir la especificación del archivo a la colección 'embedded_files' del PDF.
1. Convertir el documento a PDF/A-3A usando 'document.convert', especificando:
- Ruta del archivo de registro.
- Formato objetivo - 'ap.PdfFormat.PDF_A_3A'.
- Acción de error - 'ap.ConvertErrorAction.DELETE' para eliminar elementos no compatibles.
1. Guardar el PDF convertido en la ruta de salida.
...
Esta función convierte un PDF al formato PDF/A-1B mientras gestiona fuentes faltantes sustituyéndolas por las disponibles. Esto asegura que el PDF convertido mantenga la consistencia visual y cumpla con los estándares de archivo.

1. Cargar el PDF usando 'ap.Document'.
1. Convertir el PDF a PDF/A-1B usando 'document.convert', especificando:
- Ruta del archivo de registro.
- Formato objetivo - 'ap.PdfFormat.PDF_A_1B'.
- Acción de error - 'ap.ConvertErrorAction.DELETE' para eliminar elementos no compatibles.
1. Guardar el PDF convertido en la ruta de salida.
1. Imprimir un mensaje de confirmación.
...
- Activar 'enable_auto_tagging = True'.
- Establecer 'heading_recognition_strategy = AUTO' para detectar encabezados automáticamente.
1. Asignar la configuración de etiquetado automático a las opciones de conversión.
1. Convertir el PDF usando 'document.convert(options)'.
1. Guardar el PDF convertido en la ruta de salida.
1. Imprimir un mensaje de confirmación.
...
Este fragmento valida si un documento PDF se ajusta al estándar PDF/E-1, que es una norma ISO diseñada para documentos de ingeniería y técnicos. Los resultados de la validación se guardan en un archivo de registro.

1. Cargar el documento PDF usando 'ap.Document'.
1. Llamar al método de validación, especificando:
- Ruta del archivo de registro para almacenar los resultados de la validación.
...
El siguiente ejemplo muestra cómo convertir un PDF al formato PDF/E-1, que es una norma ISO diseñada para documentación de ingeniería y técnica. Este formato preserva el diseño preciso, los gráficos y los metadatos requeridos para flujos de trabajo de ingeniería.
...
- Acción de error - 'ap.ConvertErrorAction.DELETE' para eliminar elementos no compatibles.
1. Convertir el PDF usando 'document.convert(options)'.
1. Guardar el PDF convertido en la ruta de salida especificada.
1. Imprimir un mensaje de confirmación.
...
El siguiente fragmento de código convierte un documento PDF al formato PDF/X-4, que es una norma ISO utilizada comúnmente en la industria de impresión y publicación. PDF/X-4 garantiza la precisión del color, mantiene la transparencia e incrusta perfiles ICC para una salida consistente en todos los dispositivos.
...
1. Proporcione el **archivo de perfil ICC** para la gestión de color mediante 'icc_profile_file_name'.
