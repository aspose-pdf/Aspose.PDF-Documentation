---
title: Add Header and Footer to PDF via Node.js via C++ 
...
1. The [AsposePdfAddTextHeaderFooter](https://reference.aspose.com/pdf/nodejs-cpp/core/asposepdfaddtextheaderfooter/) function is executed.
...
The following code snippet shows you how to add text in the header of a PDF file with Node.js.
...
```js
  ...
  var ffileAddTextHeaderFooter = function (e) {
    ...
    file_reader.onload = (event) => {
      ...
      const json = AsposePdfAddTextHeaderFooter(event.target.result, e.target.files[0].name, "Aspose.PDF for Node.js via C++", "", "ResultAddHeader.pdf");
...
```js
    ...
    const ffileAddTextHeaderFooter = e => {
      const file_reader = new FileReader();
      file_reader.onload = event => {
        const header = 'Aspose.PDF for Node.js via C++';
        const footer = 'ASPOSE';
