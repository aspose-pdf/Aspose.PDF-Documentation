...
Pour utiliser Aspose.PDF for JasperReports dans votre application, copiez **aspose.pdf.jasperreports.jar** du dossier \lib du **Aspose.PDF.JasperReports.zip** vers le répertoire JasperReports\lib ou dans un dossier de bibliothèque de votre application. Après cela, vous pouvez accéder aux exportateurs par programmation.

{{% /alert %}}

L'exemple suivant montre le code typique nécessaire pour exporter un rapport au format PDF à l'aide d'Aspose.PDF for JasperReports. D'autres exemples peuvent être trouvés dans les rapports de démonstration inclus dans le téléchargement du produit.
