...
En copiant, Tomcat8 créera automatiquement un nouveau dossier " **JavaBridge**" dans **webapps**. Une fois le dossier créé, assurez-vous que votre Tomcat8 est en cours d'exécution, puis ouvrez http://localhost:8080/JavaBridge dans le navigateur, il devrait ouvrir une page par défaut de JavaBridge.
...
Après avoir installé php5.5 CGI, redémarrez le serveur Tomcat8 et ouvrez à nouveau http://localhost:8080/JavaBridge  dans le navigateur.

Si une erreur **JAVA_HOME** s'affiche, ouvrez le fichier /etc/default/tomcat8 et décommentez la ligne qui définit JAVA_HOME. Ouvrez à nouveau http://localhost:8080/JavaBridge  dans le navigateur, il devrait contenir la page d'exemples PHP/JavaBridge.
...
## Configurer le code source sous Windows
