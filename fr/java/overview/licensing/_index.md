...
sitemap:
    ...
    priority: 0.7
TechArticle: true
AlternativeHeadline: Licence d'Aspose.PDF for Java
Abstract: Cet article présente les limitations de la version d’évaluation et les options de licence d’Aspose.PDF for Java. Les exemples montrent comment charger une licence à partir d’un fichier ou d’un flux, vérifier son application, configurer une licence à la consommation et utiliser plusieurs produits Aspose.
---
...
Vous pouvez télécharger une version d'évaluation de **Aspose.PDF for Java** à partir de [Aspose Repository](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-pdf). La version d'évaluation offre absolument les mêmes fonctionnalités que la version sous licence du produit. De plus, la version d'évaluation devient simplement sous licence lorsque vous achetez une licence et ajoutez quelques lignes de code pour appliquer la licence.
...
La licence peut être chargée à partir d'un fichier ou d'un objet flux.
...
Utilisez la méthode `License.setLicense` pour appliquer une licence au composant. Souvent, le moyen le plus simple de définir une licence consiste à placer le fichier de licence dans le même dossier que Aspose.PDF.jar et à spécifier uniquement le nom du fichier sans chemin, comme indiqué dans l'exemple suivant :
...
Pour vérifier si la licence a été appliquée, utilisez la méthode `isLicensed` de la classe `Document`. Elle renvoie `true` si la licence est correctement définie.
...
Aspose.PDF permet aux développeurs d'appliquer une clé de licence à la consommation. Il s'agit d'un nouveau mécanisme de licence. Le nouveau mécanisme de licence sera utilisé parallèlement à la méthode de licence existante. Les clients qui souhaitent être facturés en fonction de l'utilisation des fonctionnalités de l'API peuvent utiliser la licence à la consommation. Pour plus de détails, veuillez vous référer à la section [FAQ sur les licences à la consommation](https://purchase.aspose.com/faqs/licensing/metered).

Une nouvelle classe [Metered](https://reference.aspose.com/pdf/java/com.aspose.pdf/Metered) a été introduite pour appliquer une clé de licence à la consommation. Voici un exemple de code montrant comment définir les clés publique et privée d’une licence à la consommation.
...
## Utiliser plusieurs produits Aspose
