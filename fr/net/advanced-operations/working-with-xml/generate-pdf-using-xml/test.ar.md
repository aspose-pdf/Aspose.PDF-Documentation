...
description: يوفر Aspose.PDF for .NET عدة طرق لتحويل ملف XML إلى مستند PDF يتطلب ملف XML المدخل.
lastmod: "2022-02-17"
sitemap:
    changefreq: "weekly"
    priority: 0.7
...
<script type="application/ld+json">
{
    ...
    "keywords": "pdf, c#, توليد pdf من xml, تحويل xml إلى pdf",
    "wordcount": "302",
...
يعمل المقتطف التالي أيضًا مع مكتبة [Aspose.PDF.Drawing](/pdf/fr/net/drawing/).

إن إنشاء مستند PDF من مستند XML ليس مهمة سهلة لأن مستند XML يمكن أن يصف محتوى مختلف. يحتوي Aspose.PDF for .NET على عدة طرق لإنشاء PDF بناءً على مستند XML:
...
XSL (لغة الأنماط القابلة للتوسيع) هي لغة تنسيق لتحويل مستندات XML إلى مستندات XML أخرى أو HTML. في حالتنا، يمكننا استخدام تحويل XML إلى HTML ثم إنشاء PDF بناءً على بيانات HTML.
...
So, we need to transform XML and load into PDF document. يظهر المثال التالي هذه الطريقة:
...
XSL-FO هي لغة ترميز تعتمد على XML لوصف تنسيق بيانات XML للإخراج على الشاشة أو الورق أو الوسائط الأخرى. يحتوي Aspose.PDF على فئة خاصة تتيح تطبيق ترميز XSL-FO والحصول على مستند PDF.

لنتناول مثالاً. هنا ملف XML يحتوي على بيانات نموذجية للموظفين.
...
المقتطف التالي يظهر كيفية استخدام هذه الفئة مع الملفات النموذجية المشار إليها أعلاه.
...
أحيانًا نحتاج إلى استخدام [XSL:param](https://developer.mozilla.org/en-US/docs/Web/XSLT/Element/param). عنصر `<xsl:param>` يقوم بتحديد معلمة بالاسم، واختياريا، قيمة افتراضية لتلك المعلمة.

دعونا نأخذ نفس المثال كما في الحالة السابقة، ولكن مع تغييرات طفيفة (إضافة المعلمات). ملف XML مع البيانات النموذجية يبقى دون تغيير، ...
```xml
<?xml version="1.0" encoding="utf-8" ?>
<employees>
...
لكن في ملف XSL-FO سنضيف معلمة: `<xsl:param name="isBoldName"></xsl:param>` وسوف نطبقها على عمود `Name`.
...
طريقة أخرى لإنشاء مستند PDF من XML هي استخدام Aspose.PDF XML Schema. باستخدام هذا المخطط، يمكنك وصف تخطيط الصفحة بنفس الطريقة كما لو كنت تستخدم تخطيط الجدول في HTML. دعونا ندرس عمل هذه الطريقة بمزيد من التفصيل.

### تعريف الصفحة

دعونا نعرف الصفحة بالمعايير الافتراضية. ستكون صفحتنا بحجم A4 وستحتوي على قطعة نصية واحدة فقط.
...
لتعريف حجم صفحة جديد يجب علينا إضافة عنصر `PageInfo`. في المثال التالي، قمنا بتحديد حجم الصفحة A5 والهوامش 25mm و10mm.
...
نظرًا لأن HTML يحتوي على علامات مشابهة لـ XML، فعندما تكتب HTML داخل أي علامة XML، يعاملها المحلل كعلامات XML ولا يمكن التعرف عليها كعلامات XML. يمكن التغلب على المشكلة باستخدام قسم "CDATA" في XML. يحتوي قسم CDATA على نص لا يتم تحليله بواسطة المحلل أو بمعنى آخر، لا يتم معاملته كعلامات XML. يظهر قالب XML النموذجي التالي كيفية إضافة HtmlFragment داخل علامات XML باستخدام CDATA.
...
تُستخدم العناصر `Table`، `Row`، `Cell` لوصف الجداول. يظهر المقتطف التالي استخدام جدول بسيط. في هذا المثال، تحتوي بعض الخلايا على سمة `Alignment` وهذه السمة لها قيمة رقمية:

1. محاذاة إلى اليسار
1. محاذاة إلى الوسط
1. محاذاة إلى اليمين.
1. محاذاة على الجانبين. سيتم محاذاة النص على الهامشين الأيسر والأيمن. ```xml
<?xml version="1.0" encoding="utf-8" ?>
<Document xmlns="Aspose.Pdf">
  <Page>
    ...
    <HtmlFragment>
      <![CDATA[
        <h1 style="font-family:Tahoma; font-size:16pt;">الجداول الزمنية على طريق جرينتاون-بلو بيرغ</h1>
        ]]>
    </HtmlFragment>
    <TextFragment>
    ...
    <Table ColumnAdjustment="AutoFitToWindow" ColumnWidths ="10 10 10 10">
      ...
      <Row BackgroundColor="LightGray" MinRowHeight="20">
        ...
        <Cell Alignment="2">
          <TextFragment>
            <TextSegment>مغادرة</TextSegment>
          </TextFragment>
        </Cell>
        <Cell>
          <TextFragment>
            <TextSegment>وصول</TextSegment>
          </TextFragment>
        </Cell>
        <Cell>
          <TextFragment>
            <TextSegment>يوم من الأسبوع</TextSegment>
          </TextFragment>
        </Cell>
        <Cell>
          <TextFragment>
            <TextSegment>سفينة</TextSegment>
          </TextFragment>
        </Cell>
      </Row>
      <Row>
...
```
تُستخدم الجداول لتنسيق المستندات. على سبيل المثال، يمكننا تخصيص رأس الصفحة. في هذه الحالة، استُخدمت الجداول لتقسيم الرأس إلى عمودين.
...
<Document xmlns="Aspose.Pdf">
  <Page>
    ...
    <Header>
        ...
        <Table ColumnAdjustment="AutoFitToWindow">
            <Row>
                ...
                <Cell Alignment="3">
                    <TextFragment>
                        <TextSegment>الصفحة $p / $P</TextSegment>
                    </TextFragment>
                </Cell>
            </Row>
        </Table>
    ...
    <HtmlFragment>
      <![CDATA[
        <h1 style="font-family:Tahoma; font-size:16pt;">الجداول الزمنية على خط جرينتاون-بلو بيرغ</h1>
        ]]>
    </HtmlFragment>
    <TextFragment>
      <TextSegment>4.1.-28.3.2021 | جرينتاون → بلو بيرغ</TextSegment>
...
توفر طريقة BindXML() ميزة تحميل محتويات ملف XML ويمكن استخدام طريقة Document.save() لحفظ النتيجة بتنسيق PDF. ومع ذلك، أثناء التحويل، يمكننا أيضًا الوصول إلى العناصر الفردية داخل XML واستخدام XML كقالب. يظهر مقتطف الشفرة التالي الخطوات للوصول إلى TextSegments من ملف XML.
...
يمكننا إضافة عناصر إضافية أخرى إلى مستند XML: كائنات الصورة أو الرسم البياني. يوضح المقتطف التالي كيفية إضافة تلك العناصر إلى المستند
