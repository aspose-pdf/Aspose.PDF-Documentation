...
description: 此页面介绍了最近版本中在 Aspose.PDF for Python via .NET 中推出的最受欢迎的新功能。
sitemap: 
    changefreq: "monthly"
    priority: 0.8
lastmod: "2025-02-24"
...
新版本 25.9 引入了改进的可访问性、增强的合规支持以及用于处理标记图像和文档标准的新 API 功能。
...
在 Aspose.PDF for Python 库的 25.9 版本中，提供了 PDF/E-1 格式转换。您可以在 [(文件格式文档)[https://docs.fileformat.com/pdf/e/]] 上找到有关此格式的更多信息。
...
在 PDF 中从流添加标记图像。25.9 版本通过从内存流添加图像并使用替代文本进行标记，支持增强的 PDF 可访问性。
...
1. 创建标记的目录 (TOC)。
1. 在内容缩放的情况下调整 PDF 页面大小。
1. 为表格应用虚线边框。

### 创建标记的目录 (TOC)

在标记的 PDF 中自动生成可访问的目录 (TOC)。在 PDF 中创建完整可访问的目录 (TOC) 可让读者高效导航文档，并确保符合 PDF/UA-1 的可访问性标准。
...
在保持布局的同时调整 PDF 页面大小并按比例缩放内容。在处理 PDF 时，您可能需要调整页面大小或缩放内容以适应新尺寸。
...
使用虚线为表格添加自定义边框样式。此示例演示了如何使用 Aspose.PDF for Python via .NET 在 PDF 文档的表格中应用自定义边框样式（例如虚线或点线）。
...
使用基于公钥证书的加密来保护您的 PDF。公钥证书加密允许为特定接收者加密 PDF，确保只有对应私钥的持有者才能打开和阅读文档。
...
自动缩放文本以适应定义的矩形内部。当在 PDF 中更新或扩展文本时，可能会超出原始段落的范围。
...
通过云或多边形样式的批注来增强 PDF 审核工作流。多边形批注允许您使用几何形状在 PDF 中突出或强调特定区域。
...
通过 LicenseInfo 检索详细的许可证元数据（许可用户、过期日期）。
...
使用 SetTextStyle 将粗体、斜体、下划线等样式应用于批注文字，或清除已有的格式。
...
最新的 Aspose.PDF 更新引入了多项强大增强功能，提升文档可访问性、兼容性和安全性。开发者现在可以直接从已签名的 PDF 文件中提取数字证书，实现高级验证和合规检查。

1. 从 PDF 签名中提取证书。
1. 在标签 PDF 中创建结构化有序列表。
1. 使用公钥证书验证签名。
1. 将动态 XFA 表单转换为 AcroForm PDF。
1. PDF 到 XPS 转换中的字体替换。
...
用默认回退字体（例如 “Courier New”）替代缺失的字体。
...
将 PDF 转换为 PDF/A-1b，并自动创建逻辑结构，以提升可访问性。
