...
我们可以通过从单个页面或整个文档中搜索特定文本（使用"纯文本"或"正则表达式"）来从PDF文档中获取文本，或者我们可以获取单个页面、页面范围或整个文档的完整文本。
 在某些情况下，您需要从 PDF 文档中提取段落或以段落形式的文本。我们已实现功能，用于在 PDF 文档页面的文本中搜索章节和段落。我们引入了 ParagraphAbsorber 类（类似于 TextFragmentAbsorber 和 TextAbsorber），可以用来从 PDF 文档中提取段落。您可以通过以下两种方式使用 ParagraphAbsorber：

**1- 通过在 PDF 页面上绘制文本的章节和段落的边框：**
