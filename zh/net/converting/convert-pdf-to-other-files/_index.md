...
本文解释了如何使用 C# 将 PDF 转换为不同格式。它涵盖以下主题。

- [将 PDF 转换为 EPUB](#csharp-convert-pdf-to-epub)
- [将 PDF 转换为 LaTeX/TeX](#csharp-convert-pdf-to-latex)
- [将 PDF 转换为纯文本](#csharp-convert-pdf-to-plain-text)
- [将 PDF 转换为 XPS](#csharp-convert-pdf-to-xps)
- [将 PDF 转换为 Markdown](#csharp-convert-pdf-to-markdown)
...
<a name="csharp-convert-pdf-to-epub" id="csharp-convert-pdf-to-epub"><strong>将 PDF 转换为 EPUB</strong></a>
...
EPUB 旨在支持可重排内容，这意味着 EPUB 阅读器可以针对特定显示设备优化文本。EPUB 还支持固定布局内容。该格式旨在作为出版商和转换公司可以在内部使用的单一格式，以及用于分发和销售。它取代了开放电子书标准。
...
<a name="csharp-convert-pdf-to-latex" id="csharp-convert-pdf-to-latex"><strong>将 PDF 转换为 LaTeX/TeX</strong></a>
...
<a name="csharp-convert-pdf-to-plain-text" id="csharp-convert-pdf-to-plain-text"><strong>将 PDF 转换为纯文本</strong></a>
...
您可以使用 Aspose.PDF for .NET 将 PDF 文档转换为 TXT 文件。您应该使用 `TextAbsorber` 类的 `Visit` 方法来解决此任务。
...
<a name="csharp-convert-pdf-to-xps" id="csharp-convert-pdf-to-xps"><strong>将 PDF 转换为 XPS</strong></a>

**Aspose.PDF for .NET** 提供将 PDF 文件转换为 <abbr title="XML Paper Specification">XPS</abbr> 格式的可能性。请尝试使用所提供的代码片段将 PDF 文件转换为 XPS 格式。
...
要将 PDF 文件转换为 XPS，Aspose.PDF 提供了类 [XpsSaveOptions](https://reference.aspose.com/pdf/zh/net/aspose.pdf/xpssaveoptions)，该类用作 [Document.Save(..)](https://reference.aspose.com/pdf/zh/net/aspose.pdf/document/methods/save/index) 方法的第二个参数，以生成 XPS 文件。

自 24.2 版本以来，Aspose.PDF 实现了将可搜索 PDF 转换为 XPS，同时保持文本在生成的 XPS 中可选择。为了保留文本，必须将 XpsSaveOptions.SaveTransparentTexts 属性设置为 true。
...
<a name="csharp-convert-pdf-to-markdown" id="csharp-convert-pdf-to-markdown"><strong>将 PDF 转换为 Markdown</strong></a>

**Aspose.PDF for .NET** 提供将 PDF 文件转换为 <abbr title="Markdown">MD</abbr> 格式的可能性。请尝试使用所提供的代码片段将 PDF 文件转换为 MD 格式。
...
由于 markdown 文件无法设置图像大小，因此在没有 MarkdownSaveOptions.UseImageHtmlTag 选项的情况下，相同类型的不同大小的图片将被保存为不同的。

启用选项 MarkdownSaveOptions.UseImageHtmlTag 将保存唯一图像，这些图像在文档中通过 img 标签进行缩放。
