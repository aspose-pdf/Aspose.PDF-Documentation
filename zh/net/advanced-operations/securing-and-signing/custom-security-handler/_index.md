---
title: 使用自定义安全处理程序在 C# 中添加数字签名
linktitle: 自定义安全处理程序
type: docs
weight: 40
url: /zh/net/custom-security-handler/
description: 使用自定义安全处理程序在 C# 中添加数字签名
lastmod: "2025-04-08"
sitemap:
    changefreq: "weekly"
    priority: 0.7
...
<script type="application/ld+json">
{
    ...
    "abstract": "Aspose.PDF for .NET 引入了强大的功能，可以使用 C# 或 VB.NET 对 PDF 文档进行数字签名，增强文档的完整性和安全性。用户可以实现各种签名类型，包括非分离和分离签名，支持 PKCS7 和 ECDSA，允许根据特定的加密标准定制签名过程。此功能不仅验证文档的真实性，还支持时间戳和认证，确保可信的文档处理。",
    "author": {
        "@type": "Person",
    ...
    "description": "使用 C# 或 VB.NET 对 PDF 文档进行数字签名。使用 C# 或 VB.NET 验证或验证数字签名的 PDF。"
}
</script>

您可以通过应用所需的加密算法来创建自己的安全处理程序。
Adobe Acrobat 将无法打开此类文件，但您可以使用 Aspose.Pdf 进行处理。
要创建处理程序，您应该实现接口：
