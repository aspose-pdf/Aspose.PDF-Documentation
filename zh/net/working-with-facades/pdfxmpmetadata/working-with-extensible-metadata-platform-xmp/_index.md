...
可扩展元数据平台 (XMP) 是由 Adobe Systems Inc. 创建的标准。该标准旨在处理和存储标准化和专有元数据。这些元数据可以嵌入到不同的文件格式中，但在本文中我们将仅关注 PDF 文件格式。我们将看到如何使用 Aspose.Pdf.Facades 命名空间在 PDF 文件中嵌入元数据。我们将使用 [PdfXmpMetadata](https://reference.aspose.com/pdf/zh/net/aspose.pdf.facades/pdfxmpmetadata) 类来操作 PDF 文档中的 XMP。
...
以下代码片段将帮助您理解如何使用 [PdfXmpMetadata](https://reference.aspose.com/pdf/zh/net/aspose.pdf.facades/pdfxmpmetadata) 类来处理 XMP：
