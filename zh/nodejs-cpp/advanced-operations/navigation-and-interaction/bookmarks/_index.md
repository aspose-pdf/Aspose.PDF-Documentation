---
title: Node.js 中的 PDF 书签
linktitle: PDF 书签
type: docs
weight: 10
url: /zh/nodejs-cpp/bookmark/
description: 您可以在 Node.js 环境中添加或删除 PDF 文档的书签。
lastmod: "2026-07-18"
sitemap:
    changefreq: "weekly"
    priority: 0.7
...
您可以使用 Aspose.PDF for Node.js via C++ 删除 PDF 文件中的书签。如果您想从 PDF 中删除书签，可以使用 [AsposePdfDeleteBookmarks](https://reference.aspose.com/pdf/nodejs-cpp/organize/asposepdfdeletebookmarks/) 函数。 
请检查以下代码片段，以在 Node.js 环境中删除 PDF 文件的书签。

**CommonJS:**

1. 调用 `require` 并导入 `asposepdfnodejs` 模块 作为 `AsposePdf` 变量。
1. 指定要从中删除书签的 PDF 文件的名称。
1. 调用 `AsposePdf` 作为 Promise 并执行删除书签的操作。如果成功，接收该对象。
1. 调用函数 [AsposePdfDeleteBookmarks](https://reference.aspose.com/pdf/nodejs-cpp/organize/asposepdfdeletebookmarks/).
1. 删除书签。因此，如果 'json.errorCode' 为 0，操作结果将保存为 "ResultPdfDeleteBookmarks.pdf"。如果 json.errorCode 参数不为 0，且相应地在文件中出现错误，错误信息将包含在 'json.errorText' 中。
