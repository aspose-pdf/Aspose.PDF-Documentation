...
## 使用用户密码或拥有者密码加密 PDF 文件

如果您要向某人发送包含机密信息的 PDF 附件的电子邮件，您可能希望先为其添加一些安全措施，以防止其落入不法之徒手中。限制对 PDF 文档的未授权访问的最佳方法是使用密码进行保护。要轻松且安全地加密文档，您可以使用 Aspose.PDF for Node.js via C++。

>在加密 PDF 文件时，请指定不同的用户密码和拥有者密码。

- 如果已设置，**用户密码** 是打开 PDF 所需提供的密码。Acrobat/Reader 会提示用户输入用户密码。如果密码不正确，文档将无法打开。
- 如果已设置，**所有者密码** 控制权限，如打印、编辑、提取、注释等。Acrobat/Reader 将根据权限设置禁止这些操作。如果您想设置/更改权限，Acrobat 将需要此密码。

如果您想加密 PDF 文件，可以使用 [AsposePdfEncrypt](https://reference.aspose.com/pdf/nodejs-cpp/security/asposepdfencrypt/) 函数。 
如果您想加密 PDF 文件，请尝试以下代码片段：

**CommonJS:**

1. 调用 `require` 并导入 `asposepdfnodejs` 模块 作为 `AsposePdf` 变量。
1. 指定将被加密的 PDF 文件名称。
1. 调用 `AsposePdf` 作为 Promise 并执行文件加密操作。如果成功，则接收对象。
