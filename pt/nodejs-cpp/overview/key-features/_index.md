---
title: Características Principais do Aspose.PDF para Node.js
linktitle: Características Principais
type: docs
weight: 20
url: /pt/nodejs-cpp/key-features/
description: Aspose.PDF para Node.js via C++ demonstra suas características gerais. Esta página descreve as versões de PDF suportadas e todas as manipulações que podemos fazer com texto, imagens, páginas, etc.
lastmod: "2023-11-16"
sitemap:
    changefreq: "weekly"
    priority: 0.7
---

## Características Gerais

- **Aspose.PDF para Node.js via C++** é construído com o uso da tecnologia WebAssembly e é baseado no Aspose.PDF para .NET
- Definir, Obter e Remover Informações do Arquivo PDF
- Excluir Marcadores
- Excluir Anotações
- Extrair texto de PDF
- Otimizar documento PDF
- Otimizar recursos
- Dividir PDF em dois arquivos
- Mesclar dois arquivos em PDF
- Reparar PDF

## Versões de PDF Suportadas

Aspose.PDF para Node.js suporta versões de PDF 1.2, 1.3, 1.4, 1.5, 1.6 e 1.7.

## Fontes

- 14 fontes principais.
- Fontes PostScript Type1.
- Fontes TrueType.
- Fontes Tipo 3.
- Incorporação de informações de fonte no PDF
- Fontes CJK.
- Suporte a Unicode

## Texto

- Extrair um texto de PDF

## Conversão

- Converter um arquivo PDF para DOC
- Converter um arquivo PDF para DOCX
- Converter um arquivo PDF para PPTX
- Converter um arquivo PDF para EPUB
- Converter um arquivo PDF para XLSX
- Converter um arquivo PDF para PDF em escala de cinza
- Converter um arquivo PDF para TXT
- Converter um arquivo PDF para TEX
- Converter um arquivo PDF para CSV
- Converter um arquivo PDF para PDF/A
- Converter um arquivo PDF/A para PDF
- Converter PDF para formatos de imagem

## Imagens

- Adicionar imagem em arquivo PDF
- Excluir imagens de PDF
- Conversão de PDF para JPEG
- Conversão de PDF para PNG
- Conversão de PDF para Tiff
- Conversão de PDF para BMP
- Conversão de PDF para SVG
- Conversão de PDF para SVG (Zip)

## Página

- Mesclar páginas de PDF
- Dividir páginas de PDF
- Adicionar fundos
- Adicionar números de página
- Adicionar cabeçalhos e rodapés
- Excluir páginas
- Girar páginas

## Carimbo

- Adicionar um carimbo ao PDF

## Recursos de Hyperlink e Anexos

A caixa de ferramentas permite adicionar:

- Remover anexos do PDF
- Atalhos de hyperlink para outras páginas dentro de um documento PDF
- Hyperlinks para páginas em outros documentos PDF
- Hyperlinks para arquivos não-PDF

- Hyperlinks para locais na web.

Along with hyperlink creation, you also get the capabilities to extract hyperlinks from a PDF document

## Funcionalidades de Segurança em PDF

A API oferece criptografia de 40 bits ou 128 bits. Também é possível definir senhas de usuário e mestre para maior segurança. A manipulação de conteúdo em arquivos, como copiar conteúdo, imprimir, impressão degradada, modificação de anotações, preenchimento de campos de formulário, bem como montagem de arquivos PDF, podem ser todos restringidos usando esta biblioteca.

- Criptografar arquivo PDF
- Descriptografar arquivo PDF
- Alterar senha
- Assinar PDF