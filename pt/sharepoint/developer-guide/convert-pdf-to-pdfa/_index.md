---
title: Converter PDF para PDFA no SharePoint
linktitle: Converter PDF para PDFA
type: docs
weight: 70
url: /pt/sharepoint/convert-pdf-to-pdfa/
lastmod: "2026-08-31"
description: Usando a API PDF SharePoint, você pode converter PDF para o formato PDFA. Atualmente, ela suporta apenas o padrão PDF/A-1b.
---

{{% alert color="primary" %}}

Em [Aspose.PDF for SharePoint 2.0](https://releases.aspose.com/pdf/sharepoint/new-releases/aspose.pdf-for-sharepoint-2.0.0/) Lançamento: adicionamos suporte para criar PDF compatível com PDFA.

Atualmente, o Aspose.PDF for SharePoint suporta apenas o padrão PDFA1b.

{{% /alert %}}

## Criando um documento compatível com PDFA

Converta o PDF da biblioteca de documentos do SharePoint para PDFA da seguinte forma:

1. Clique **Convert to PDF** no menu ECB.

![Converter PDF para PDFA-exemplo 1](convert-pdf-to-pdfa_1.png)

2. Baixe e salve o arquivo PDF resultante.

![Converter PDF para PDFA-exemplo 2](convert-pdf-to-pdfa_2.png)
