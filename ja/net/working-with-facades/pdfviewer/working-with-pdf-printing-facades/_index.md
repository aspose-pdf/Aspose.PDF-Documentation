...
<script type="application/ld+json">
{
    ...
    "abstract": "Aspose.PDF for .NET ファサード機能は、プリンター設定や出力形式に対する制御を強化し、PDF印刷を簡素化します。ユーザーは、デフォルトまたは仮想プリンターにドキュメントをシームレスに印刷し、ページレイアウトを定義し、単面または両面モードで印刷ジョブを管理することができます。また、グレースケールで印刷したり、印刷ダイアログを非表示にするオプションがその多様性を高めています。この機能は、PDFドキュメントの印刷ワークフローを大幅に最適化し、効率的なドキュメント管理ソリューションを求める開発者やユーザーに最適です。",
    "author": {
        "@type": "Person",
...
カラーページPDFドキュメントは、[PdfViewer](https://reference.aspose.com/pdf/ja/net/aspose.pdf.facades/pdfviewer)を使用してグレースケールとしてXPSプリンターに印刷できます。それを実現するために、プロパティ[PdfViewer.PrintAsGrayscale](https://reference.aspose.com/pdf/ja/net/aspose.pdf.facades/pdfviewer/printasgrayscale/)を*true*に設定します。次のコードスニペットは、`PdfViewer.PrintAsGrayscale`プロパティの使用方法を示しています。
...
時には、関連する複数のドキュメントを単一の印刷ジョブとして一緒に印刷する必要があります。これにより、これらのドキュメントが他のユーザーの出力と混在しないように、特にリモートネットワークプリンターでの印刷が保証されます。Aspose.PDFは、[PdfViewer](https://reference.aspose.com/pdf/ja/net/aspose.pdf.facades/pdfviewer)クラスの静的`PrintDocuments`メソッドを介して、共有プリンター設定で任意の数のドキュメントを単一の印刷ジョブで印刷することをサポートしています。印刷するドキュメントは、ファイルパス、ドキュメントストリーム、または[Document](https://reference.aspose.com/pdf/ja/net/aspose.pdf/document)オブジェクトとして提供できます。

{{% alert color="primary" %}}

複数のドキュメントを印刷する場合、[PrinterSettings.PrintRange](https://reference.aspose.com/pdf/ja/net/aspose.pdf.printing/printersettings/printrange/)プロパティは無視され、すべてのドキュメントが完全に印刷されます。
