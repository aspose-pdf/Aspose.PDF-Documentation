---
title: Kelas PdfFileSecurity
type: docs
weight: 125
url: /net/pdffilesecurity-class/
description: Bagian ini menjelaskan cara bekerja dengan Aspose.PDF Facades menggunakan Kelas PdfFileSecurity.
lastmod: "2021-06-05"
draft: false
sitemap:
    changefreq: "weekly"
    priority: 0.7
---

- [Enkripsi File PDF](/pdf/net/encrypt-pdf-file/)
- [Dekripsi File PDF](/pdf/net/decrypt-pdf-file/)
- [Kontrol Pengecualian File PDF](/pdf/net/control-exception/)
- [Ganti Kata Sandi File PDF](/pdf/net/change-password/)
- [Setel Hak Istimewa pada File PDF yang Ada](/pdf/net/set-privileges/)