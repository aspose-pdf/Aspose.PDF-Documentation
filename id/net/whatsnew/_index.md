...
* opsi untuk menyimpan PDF ke HTML dengan melewati semua gambar raster.
...
Metode ekstensi `PageCollection` telah ditambahkan untuk memperbarui nomor halaman dan artefak header/footer tanggal saat menambahkan atau menyisipkan halaman baru. Pengaturan untuk nomor halaman dan format tanggal harus disimpan dalam dokumen asli sesuai dengan spesifikasi PDF, seperti yang diimplementasikan oleh Adobe Acrobat.
...
ECDSA mendukung SHA-1, SHA-256, SHA-384, SHA-512, SHA3-256, SHA3-384, dan SHA3-512. Algoritma SHA3-256, SHA3-384, dan SHA3-512 hanya didukung untuk .NET 8 dan versi yang lebih baru. Untuk detail tentang platform yang didukung untuk SHA-3, lihat [dokumentasi](https://learn.microsoft.com/en-us/dotnet/standard/security/cross-platform-cryptography#sha-3).
...
Terkadang, perlu untuk memotong gambar sebelum menyisipkannya ke dalam PDF. Kami telah menambahkan versi overload dari metode `AddImage()` untuk mendukung penambahan gambar yang dipotong:
...
## Apa yang baru di Aspose.PDF 24.9
...
Untuk mengonversi gambar HEIC ke PDF, pengguna harus menambahkan referensi ke paket NuGet `FileFormat.HEIC` dan menggunakan cuplikan kode berikut:
...
Layer.Locked; - Properti, menunjukkan status terkunci dari lapisan.
...
- Contoh dengan bidang kosong yang ada
...
- Semua formulir dapat dihapus menggunakan cuplikan kode berikut:
...
- Tanda potong yang menunjukkan di mana media output akan dipangkas
...
Jika dokumen PDF berisi teks SubScript dan SuperScript seperti H2O, maka mengekstrak teks dari PDF juga harus mengekstrak informasi format mereka (dalam teks biasa yang diekstrak).
Jika PDF berisi teks dalam huruf miring, itu juga harus disertakan dalam konten yang diekstrak.
...
### Bagaimana cara mengekstrak nama lengkap font dari deskripsi/sumbernya di PDF?
...
### Ekspos publik properti untuk mendeteksi Perlindungan Informasi Azure
