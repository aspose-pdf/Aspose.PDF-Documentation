...
description: Halaman ini memperkenalkan fitur-fitur baru paling populer dalam Aspose.PDF for Python via .NET yang telah diperkenalkan dalam rilis terbaru.
sitemap:
    changefreq: "monthly"
    priority: 0.8
lastmod: "2026-06-12"
...
Di **Aspose.PDF for Python via .NET** 26.3, kami telah menambahkan:

Rekompresi aliran gambar tanpa kehilangan selama optimasi PDF. Properti OptimizationOptions.CompressAllContentStreams kini juga mengompres aliran XObject gambar yang memenuhi syarat dengan FlateDecode, membantu mengurangi ukuran file sambil mempertahankan kualitas gambar tetap.
...
Rekompresi gambar kini cocok dengan pengaturan ImageCompressionOptions.Encoding yang dipilih selama optimisasi, memastikan hasil yang lebih konsisten saat menggunakan Jpeg2000 atau Flate, bersama dengan pengubahan ukuran gambar, batas resolusi, dan kontrol kualitas.
...
Dukungan untuk merender komentar saat menyimpan dokumen PDF sebagai gambar atau HTML, membantu mempertahankan markup ulasan yang terlihat ketika mengekspor dokumen beranotasi untuk dibagikan di luar penampil PDF.
...
Meningkatkan kinerja rendering PDF-ke-TIFF untuk skenario rasterisasi volume tinggi, terutama saat mengekspor halaman ke gambar TIFF bitonal.
...
Aspose.PDF 26.2 memperkenalkan dukungan untuk konversi RTF ke PDF. Fitur ini memungkinkan pengembang mengonversi langsung dokumen Rich Text Format (RTF) menjadi file PDF.

RTF adalah format dokumen lintas platform yang didukung secara luas dan awalnya dikembangkan oleh Microsoft. Format ini dirancang untuk memungkinkan pertukaran dokumen antara berbagai aplikasi pengolah kata sambil mempertahankan pemformatan dasar seperti font, warna, teks tebal dan miring, serta gambar yang disematkan.
...
Potongan kode ini menunjukkan cara menyisipkan tabel setelah konten yang ada pada halaman PDF menggunakan Aspose.PDF for Python.
...
Deteksi dan hapus teks tak terlihat dari dokumen PDF menggunakan Aspose.PDF untuk Python:
...
Pada **Aspose.PDF for Python via .NET** 26.1, kami telah menambahkan:

1. Peningkatan kinerja – mengatasi kinerja yang buruk saat menambahkan teks ke dokumen dan masalah kinerja secara keseluruhan.
1. Akurasi rendering yang ditingkatkan – memperbaiki teks vertikal yang hilang di sisi kiri PDF dan memperbaiki rendering karakter Cina selama konversi PDF ke PNG.
1. Konversi HTML yang ditingkatkan – API sekarang menghormati warna garis dalam konversi HTML-ke-PDF dan memperbaiki masalah tumpang tindih teks dalam konversi XFA-ke-Standard.
1. Perbaikan bug untuk struktur dokumen – NumberingStyle sekarang berfungsi dengan benar untuk objek Heading, dan PDF-to-HTML sekarang mempertahankan warna teks yang disorot.

## Apa yang baru di Aspose.PDF 25.12

Konversi dokumen HTML menjadi PDF sambil mempertahankan informasi struktur logis. PDF yang dihasilkan lebih cocok untuk aksesibilitas, penandaan, dan pemrosesan hilir yang bergantung pada konten dokumen terstruktur.
...
Analisis PDF yang ditandatangani secara digital untuk mengidentifikasi dan melaporkan konten yang tidak tercakup oleh tanda tangan. Gunakan untuk memvalidasi integritas dokumen, mengaudit PDF yang ditandatangani, dan mendeteksi modifikasi setelah penandatanganan.
...
Fungsi ini membandingkan halaman tertentu dari dua dokumen PDF dan menghasilkan perbedaan visual berdampingan. Dengan menyesuaikan opsi perbandingan dan warna, fungsi ini menyoroti perubahan yang berarti sekaligus mengabaikan perbedaan yang tidak signifikan seperti spasi putih.
...
Menghapus data tersembunyi dan merasterkan halaman pada versi 25.11.
...
Mengoptimalkan sumber daya dengan subsetting font dan kompresi aliran konten versi 25.11.
...
Kontrol Visibilitas Lapisan PDF yang Ditingkatkan – rilis ini memperkenalkan kemampuan untuk secara programatik menentukan status visibilitas awal lapisan PDF dan mengunci mereka agar tidak dapat diubah visibilitasnya di penampil PDF.

Properti 'layer.default_state' yang baru memungkinkan pengaturan visibilitas default sebuah layer menjadi visible atau hidden menggunakan properti DefaultState. Ini menyediakan kontrol yang sangat detail untuk mengelola dokumen PDF berlapis yang kompleks dengan perilaku visibilitas pengguna yang dapat diprediksi.
...
## Apa yang baru di Aspose.PDF 25.9

Rilis 25.9 memperkenalkan aksesibilitas yang lebih baik, dukungan kepatuhan yang ditingkatkan, dan kemampuan API baru untuk bekerja dengan gambar bertanda dan standar dokumen.

1. Ubah PDF menjadi format PDF/E-1.
1. Tambahkan gambar ber‑tag dari aliran memori.

### Ubah PDF ke Format PDF/E-1

Pada versi 25.9 perpustakaan Aspose.PDF for Python, konversi format PDF/E-1 tersedia. Anda dapat menemukan informasi lebih lanjut tentang format ini di [Dokumen Format File](https://docs.fileformat.com/pdf/e/).
...
### Tambahkan Gambar Bertag dari Aliran

Tambahkan Gambar Bertanda dari Stream dalam PDF. Versi 25.9 mendukung aksesibilitas yang ditingkatkan dalam dokumen PDF dengan menambahkan gambar dari stream memori dan menandainya dengan teks alternatif.
...
Pembaruan ini menambahkan lebih banyak fleksibilitas dalam pengaturan tata letak dan manajemen keamanan dokumen.

1. Buat daftar isi ber-tag (TOC).
1. Ubah ukuran halaman PDF dengan penskalaan konten.
1. Terapkan batas bergaris putus-putus pada tabel.

### Buat Daftar Isi yang Ditandai (TOC)

Secara otomatis menghasilkan daftar isi (TOC) yang dapat diakses dalam PDF yang ditandai. Membuat Daftar Isi (TOC) yang sepenuhnya dapat diakses dalam PDF memungkinkan pembaca menavigasi dokumen secara efisien dan memastikan kepatuhan PDF/UA-1 untuk aksesibilitas.
...
### Ubah Ukuran Halaman dengan Skala Konten

Ubah ukuran halaman PDF sambil mempertahankan tata letak dan menskalakan konten secara proporsional. Saat bekerja dengan PDF, Anda mungkin perlu mengubah ukuran halaman atau menskalakan konten agar sesuai dengan dimensi baru.
...
### Terapkan Garis Putus-Putus pada Tabel

Tambahkan tabel dengan gaya border khusus menggunakan garis putus-putus. Contoh ini menunjukkan cara menerapkan gaya border khusus—seperti garis putus-putus atau titik-titik—pada tabel dalam dokumen PDF menggunakan Aspose.PDF for Python via .NET.
...
Rilis 25.7 berfokus pada dukungan anotasi yang lebih baik, penyesuaian teks, dan manajemen tanda tangan digital.

1. Sesuaikan teks di dalam bentuk.
1. Enkripsi PDF menggunakan sertifikat publik.
1. Tambahkan anotasi awan dan poligon.

### Enkripsi PDF dengan Sertifikat Publik

Amankan PDF Anda dengan enkripsi berdasarkan sertifikat publik. Enkripsi sertifikat publik memungkinkan PDF dienkripsi untuk penerima tertentu, memastikan hanya pemegang kunci pribadi yang bersangkutan yang dapat membuka dan membaca dokumen.
...
### Sesuaikan Teks dalam Persegi Panjang

Secara otomatis skala teks agar pas di dalam persegi panjang yang ditentukan. Saat memperbarui atau memperluas teks dalam PDF, teks tersebut dapat melebihi batas paragraf asli.
...
### Tambahkan Anotasi Poligon Awan

Tingkatkan alur kerja peninjauan PDF dengan anotasi gaya awan atau poligon. Anotasi poligon memungkinkan Anda menyorot atau menekankan area tertentu dalam PDF menggunakan bentuk geometris.
...
Fitur utama dari rilis ini:

1. Dukungan Teks Alt Gambar.
1. Akses Info Lisensi.
1. Anotasi Teks Bebas Bergaya.
1. Penampilan Tanda Tangan Digital yang Dapat Disesuaikan.

### Dukungan Teks Alt Gambar

Atur dan ambil teks alternatif untuk gambar guna meningkatkan aksesibilitas bagi pembaca layar.
...
Ambil metadata lisensi terperinci (pengguna berlisensi, tanggal kedaluwarsa) melalui LicenseInfo.
...
### Anotasi Teks Bebas Bergaya

Gunakan SetTextStyle untuk menerapkan gaya seperti tebal, miring, garis bawah, atau menghapus pemformatan yang ada pada teks anotasi.
...
### Penampilan Tanda Tangan Digital yang Dapat Disesuaikan

Tambahkan gambar, ubah font, dan lapiskan grafik tanda tangan di atas konten latar belakang untuk branding atau konsistensi desain yang lebih baik.
...
Pembaruan Aspose.PDF terbaru memperkenalkan beberapa peningkatan kuat yang meningkatkan aksesibilitas dokumen, kompatibilitas, dan keamanan. Pengembang sekarang dapat mengekstrak sertifikat digital langsung dari file PDF yang ditandatangani, memungkinkan verifikasi lanjutan dan pemeriksaan kepatuhan.

1. Ekstrak Sertifikat dari Tanda Tangan PDF.
1. Buat Daftar Berurutan Terstruktur dalam Tagged PDFs.
1. Verifikasi Tanda Tangan dengan Sertifikat Kunci Publik.
1. Konversi Formulir XFA Dinamis ke PDF AcroForm.
1. Penggantian Font dalam PDF - Konversi XPS.

### Ekstrak Sertifikat dari Tanda Tangan PDF

Ambil sertifikat yang disematkan menggunakan 'extract_certificate()'.
...
### Buat Daftar Terurut Terstruktur dalam Tagged PDFs

Hasilkan daftar bernomor yang dapat diakses (dengan item bersarang) di dalam dokumen ber‑tag.
...
### Verifikasi Tanda Tangan dengan Sertifikat Kunci Publik

Validasi tanda tangan digital menggunakan sertifikat kunci publik eksternal.
...
### Konversi Formulir XFA Dinamis ke PDF AcroForm

Standarkan formulir XFA dengan 'ignore_needs_rendering'.
...
### Penggantian Font di PDF - Konversi XPS

Ganti font yang hilang dengan fallback default (mis., “Courier New”).
...
### Penandaan Otomatis Selama Konversi PDF/A

Konversi PDF ke PDF/A-1b dengan pembuatan struktur logis otomatis untuk meningkatkan aksesibilitas.
