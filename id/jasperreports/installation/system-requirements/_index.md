---
title: Persyaratan Sistem
linktitle: Persyaratan Sistem
type: docs
weight: 10
url: /id/jasperreports/system-requirements/
description: Pelajari tentang persyaratan sistem Aspose.PDF for JasperReports. Pastikan kompatibilitas dan kinerja optimal dengan lingkungan Anda.
lastmod: "2026-08-31"
---

{{% alert color="primary" %}}

Aspose.PDF for JasperReports memerlukan perangkat lunak berikut untuk dijalankan:

- Platform Java 2 Edisi Standar 5.0 atau lebih tinggi.
- JasperReports 6.7.0 atau lebih tinggi.
- JasperServer 6.7.0 atau lebih tinggi (jika Anda ingin menggunakan Aspose.PDF for JasperReports dengan JasperServer).

{{% /alert %}}

