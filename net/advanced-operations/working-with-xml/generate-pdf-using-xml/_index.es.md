...
Generar un documento PDF a partir de un documento XML no es una tarea trivial porque un documento XML puede describir diferentes contenidos. Aspose.PDF para .NET tiene varias formas de generar un PDF basado en un documento XML:

- usando transformación XSLT
- usando XSL-FO (Objetos de Formato XSL)
- usando su propio esquema XML de Aspose.PDF

## Generar documento PDF utilizando transformación XSLT

XSL (Lenguaje de Estilo Extensible) es un lenguaje de estilo para transformar documentos XML en otros documentos XML o HTML. En nuestro caso, podemos usar la transformación de XML a HTML y luego crear un PDF basado en datos HTML.
...
private static void ExampleXSLTtoPDF()
{
    ...
    // establecer el tamaño de la página a A5
...
```
```
## Generando documento PDF usando el marcado XSL-FO

XSL-FO es un lenguaje de marcado basado en XML que describe el formato de los datos XML para la salida en pantalla, papel u otros medios. Aspose.PDF tiene una clase especial que permite aplicar el marcado XSL-FO y obtener un documento PDF.
...
Vamos a crear otro archivo: el archivo de marcado XSL-FO para transformar los datos de los empleados en una tabla.
...
### Generar documento PDF usando el marcado XSL-FO y parámetros XSL
...
Tomemos el mismo ejemplo que en el caso anterior, pero con cambios menores (agregar parámetros). El archivo XML con datos de muestra permanece intacto, ...
...
establecerlo como propiedad en [XslFoLoadOptions](https://reference.aspose.com/pdf/net/aspose.pdf/xslfoloadoptions).
El siguiente fragmento muestra cómo usar esta clase con los archivos de muestra descritos anteriormente.
...
Otra forma de crear un documento PDF a partir de XML es utilizando el esquema XML de Aspose.PDF. Usando este diagrama, puedes describir el diseño de la página de la misma manera que si estuvieras usando un diseño de tabla en HTML. Vamos a considerar el funcionamiento de este método con más detalle.
...
Como HTML contiene etiquetas similares a XML, cuando escribes HTML dentro de cualquier etiqueta XML, el analizador lo trata como marcas XML y simplemente no pueden ser reconocidas como etiquetas XML.
```
...
1. Alineación justificada. El texto se alineará tanto en los márgenes izquierdo como derecho.
1.
```xml
<?xml version="1.0" encoding="utf-8" ?>
<Document xmlns="Aspose.Pdf">
...
Las tablas se utilizan para la disposición de documentos. Por ejemplo, podemos personalizar un encabezado de página. En este caso, se usaron tablas para dividir el encabezado en 2 columnas.
...
// Obtener referencia del objeto página desde XML
...
Podemos añadir otros elementos adicionales al documento XML: objetos Image o Graph. El siguiente fragmento muestra cómo añadir esos elementos al documento
...
El siguiente template XML contiene una etiqueta `<Image>` con una ID "testImg". En caso de que quieras establecer la ruta de la imagen desde tu código, puedes acceder al elemento Image desde el template XML durante el proceso de conversión y establecer la ruta a la dirección deseada para la imagen.
