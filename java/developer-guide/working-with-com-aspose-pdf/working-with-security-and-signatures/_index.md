---
title: Working with Security and Signatures
type: docs
weight: 70
url: /java/working-with-security-and-signatures/
---

## **This section includes the following topics**: 

lastmod: "2020-12-16"
----------------------------------------------
- [Encrypt PDF document using Encryption Types](/pdf/java/encrypt-pdf-document-using-encryption-types/)
- [Decrypt PDF file using Owner Password](/pdf/java/decrypt-pdf-file-using-owner-password/)
- [Add Digital Signature to PDF file](/pdf/java/add-digital-signature-to-pdf-file/)
- [Extract Image from Signature Field](/pdf/java/extract-image-from-signature-field/)
- [Set Privileges on an Existing PDF File](/pdf/java/set-privileges-on-an-existing-pdf-file/)
- [Determine if the Source PDF file is Password Protected](/pdf/java/determine-if-the-source-pdf-file-is-password-protected/)
